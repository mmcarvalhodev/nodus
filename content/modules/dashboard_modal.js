...
console.log('[Dashboard Modal] 🚀 Arquivo sendo carregado...');
...
const NodusDashboard = {
    ...
    createDashboardDOM() {
        ...
        tabs.innerHTML = `
            ...
            <button class="nodus-tab-btn" data-tab="diagnostic" title="Runtime diagnostics (v5.0.0)">
                <span class="tab-icon">🩺</span>
                <span class="tab-label">Diagnostic</span>
            </button>
        `;
        ...
        console.log('[Dashboard] DOM structure created');
    },
    ...
    attachEventListeners() {
        ...
        window.addEventListener('nodus-license-changed', (e) => {
            console.log('[Dashboard] License changed:', e.detail);
    ...
    async open(initialTab = 'cards') {
        console.log('[Dashboard] Opening...', { initialTab });
    ...
    togglePin() {
        ...
        if (this.isPinned) {
            ...
            console.log('[Dashboard] Unpinned');
        }
     ...
     */
    switchTab(tabName) {
        console.log('[Dashboard] Switching tab to:', tabName);
        ...
        switch(tabName) {
            ...
            case 'mindmap':
                ...
                break;
            case 'diagnostic':
                // Diagnostic panel (Fase 2 — runtime dinâmico health visibility)
                this.updateModalWidth(1);
                if (window.NodusDashboardDiagnostic) {
                    window.NodusDashboardDiagnostic.render(contentArea);
                } else {
                    contentArea.innerHTML = '<div style="text-align:center; padding:60px 20px; color:#a0aec0;">🩺 Diagnostic module não carregado.</div>';
                }
                break;
        }
    ...
    applyZoomCompensation() {
        ...
        if (zoom > 1.05) {
            ...
            console.log(`[Dashboard] Zoom compensated: browser=${Math.round(zoom * 100)}% → zoom=${compensation.toFixed(3)}`);
        } else {
    ...
    _pushPlatformLayout(width) {
        ...
        console.log(`[Dashboard] Platform pushed: ${w}px`);
    },
    ...
    _restorePlatformLayout() {
        ...
        console.log('[Dashboard] Platform layout restored');
    },
    ...
    updateModalWidth(columns) {
        console.log('[Dashboard] Updating modal width for', columns, 'columns');
        ...
        console.log('[Dashboard] Modal width set to', this.modal.style.width, 'cols=', columns, 'zoom=', zoom);
    ...
    async openSettings() {
        ...
        document.getElementById('nodus-settings-onboarding-btn').addEventListener('click', async () => {
            ...
            try {
                ...
                console.log('[Settings] Onboarding resetado.');
            } catch (e) {
        ...
        telemetryToggle.addEventListener('change', async (e) => {
            ...
            // ORDEM CRÍTICA: emitir o evento ANTES de persistir a flag.
            // Se invertêssemos (marca off → tenta enviar), o sendBatch
            // respeitaria a flag nova e descartaria o evento. Background
            // faz flush IMEDIATO desse evento.
            try {
                await chrome.runtime.sendMessage({
                    type: 'telemetry_toggle',
                    payload: { enabled, source: 'dashboard' }
                });
            } catch (err) {
                console.warn('[Settings] toggle event send falhou:', err?.message);
            }
            ...
            console.log(`[Settings] Telemetria ${enabled ? 'ativada' : 'desativada'}`);
        ...
        document.getElementById('btnSendTelemetryNow').addEventListener('click', async () => {
            console.log('[Settings] 🔵 Send Now button clicked!');
            
            const btn = document.getElementById('btnSendTelemetryNow');
            btn.disabled = true;
            btn.innerHTML = '⏳ Sending...';
            
            console.log('[Settings] Sending message to background...');
    ...
    async clearActivityLogs() {
        try {
            ...
            console.log('[Settings] Logs cleared. Backup saved:', currentLogs.length, 'events');
    ...
    async initializeAccountSection() {
        ...
        if (!hasLicenseData) {
            ...
            console.log('[Account] 📊 License data:', { expiresAt, renewsAt, license });
    ...
    async initializeBackupSection() {
        ...
        console.log('[Backup] 🎯 Registrando event listeners (PRO confirmado)');
        ...
        togglePasswordBtn.addEventListener('click', () => {
            console.log('[Backup] Toggle password visibility');
        ...
        console.log('[Backup] ✅ Event listener de Export registrado');
        ...
        importBtn.addEventListener('click', () => {
            ...
            console.log('[Backup] 🔍 Password no input:', password ? `(${password.length} chars)` : 'VAZIO');
            
            if (!password || password.length < 8) {
                console.log('[Backup] ❌ Senha insuficiente para import');
            ...
            console.log('[Backup] ✅ Senha OK - abrindo file picker');
    ...
    async exportEncryptedBackup() {
        console.log('[Backup] 🔐 exportEncryptedBackup iniciado');
        
        // Verificar licença PRO
        const licenseData = await chrome.storage.local.get('nodus_license');
        const license = licenseData.nodus_license || {};
        console.log('[Backup] License status:', license.status);
        
        if (license.status !== 'pro') {
            console.log('[Backup] ❌ Não é PRO - abortando');
        ...
        console.log('[Backup] Password input encontrado:', !!passwordInput);
        console.log('[Backup] Password length:', password?.length || 0);
        
        if (!password || password.length < 8) {
            console.log('[Backup] ❌ Senha inválida');
        ...
        try {
            console.log('[Backup] ✅ Iniciando criação do backup...');
            window.NODUS_UI?.showToast('🔐 Criando backup criptografado...', 'info');
            
            // Get all data
            console.log('[Backup] 📦 Coletando dados do storage...');
            const data = await chrome.storage.local.get(null);
            const dataStr = JSON.stringify(data);
            console.log('[Backup] 📦 Dados coletados:', dataStr.length, 'bytes');
            
            // Encrypt
            console.log('[Backup] 🔐 Criptografando...');
            const encrypted = await this.encryptData(dataStr, password);
            console.log('[Backup] ✅ Criptografia concluída');
            ...
            console.log('[Backup] 💾 Gerando arquivo para download...');
            // Download
            ...
            console.log('[Backup] 💾 Disparando download...');
            a.click();
    ...
    async importEncryptedBackup(event) {
        ...
        try {
            console.log('[Backup] 🔓 Iniciando import...');
            window.NODUS_UI?.showToast('🔓 Descriptografando backup...', 'info');
            
            console.log('[Backup] 📄 Lendo arquivo...');
            const text = await file.text();
            console.log('[Backup] 📄 Arquivo lido:', text.length, 'bytes');
            
            console.log('[Backup] 🔍 Parseando JSON...');
            const backup = JSON.parse(text);
            console.log('[Backup] ✅ JSON válido. Versão:', backup.version);
            
            // Decrypt
            console.log('[Backup] 🔐 Tentando descriptografar com senha fornecida...');
            const decrypted = await this.decryptData(backup.data, password, backup.salt, backup.iv);
            console.log('[Backup] ✅ Descriptografia bem-sucedida!');
            
            console.log('[Backup] 🔍 Parseando dados descriptografados...');
            const data = JSON.parse(decrypted);
            
            console.log('[Backup] ✅ Backup descriptografado com sucesso');
            console.log('[Backup] Dados no backup:', Object.keys(data).length, 'keys');
            ...
            if (isWrongPassword) {
                console.log('[Backup] ❌ Senha incorreta - import abortado');
            } else {
                console.warn('[Backup] ⚠️ Erro ao importar backup:', error.message || 'arquivo inválido');
            }
            
            if (isWrongPassword) {
                console.log('[Backup] ❌ Senha incorreta detectada');
                ...
                console.log('[Backup] 🔔 Mostrando toast de arquivo inválido');
    ...
    async executeRestore(backupData, passwordInput, mode) {
        try {
            ...
            if (mode === 'clean') {
                ...
                await chrome.storage.local.set(backupData);
                console.log('[Backup] ✅ Backup restaurado (sobrepor)');
    ...
    async showTelemetryStats(data) {
        console.log('[Dashboard] Showing telemetry stats...');
...
console.log('========================================');
console.log('[Dashboard Modal] ✅ Module loaded successfully!');
console.log('[Dashboard Modal] window.NodusDashboard =', window.NodusDashboard);
console.log('[Dashboard Modal] NodusDashboard.open =', typeof NodusDashboard.open);
console.log('[Dashboard Modal] NodusDashboard.isOpen =', NodusDashboard.isOpen);
console.log('========================================');
