...
const NodusAttachmentsDragBar = {
     ...
     */
    init() {
        console.log('[DragBar] Initializing...');
        this.createDragBar();
        this.setupDragBarEvents();
        console.log('[DragBar] Initialized!');
    },
    ...
    createDragBar() {
        ...
        console.log('[DragBar] Element created');
    },
    ...
    setupDragBarEvents() {
        ...
        this.dragBarElement.addEventListener('dragstart', (e) => {
            console.log('[DragBar] Drag start');
            ...
            if (this.loadedFiles && this.loadedFiles.length > 0) {
                console.log('[DragBar] Using preloaded files:', this.loadedFiles.length);
                
                try {
                    for (const file of this.loadedFiles) {
                        e.dataTransfer.items.add(file);
                        console.log('[DragBar] Added file to dataTransfer:', file.name);
                    }
                    
                    console.log('[DragBar] ✅ All files added to dataTransfer');
                } catch (error) {
    ...
    updateSelection(files) {
        console.log('[DragBar] Updating selection:', files.length, 'files');
    ...
    async preloadFiles(files) {
        console.log('[DragBar] Preloading', files.length, 'files from DB...');
        console.log('[DragBar] Files to preload:', files);
        this.loadedFiles = [];
        
        if (window.NodusAttachmentsDB) {
            try {
                for (const fileInfo of files) {
                    console.log('[DragBar] Loading file:', fileInfo.fileName, 'id:', fileInfo.id);
                    const fileData = await window.NodusAttachmentsDB.getFile(fileInfo.id);
                    console.log('[DragBar] FileData received:', fileData ? 'Yes' : 'No');
                    
                    if (fileData && fileData.fileData) {
                        console.log('[DragBar] Has fileData, size:', fileData.fileData.byteLength);
                        
                        // Converter ArrayBuffer para Blob
                        const blob = new Blob([fileData.fileData], { type: fileInfo.fileType });
                        console.log('[DragBar] Blob created, size:', blob.size);
                        ...
                        console.log('[DragBar] ✅ Preloaded file:', fileInfo.fileName, 'lastModified:', file.lastModified);
                    } else {
                        console.warn('[DragBar] ❌ No fileData for file:', fileInfo.fileName);
                    }
                }
                
                console.log('[DragBar] ✅ Preload complete:', this.loadedFiles.length, 'files');
            } catch (error) {
    ...
    render() {
        ...
        console.log('[DragBar] Rendered', count, 'files with thumbs');
    },
    ...
    show() {
        ...
        console.log('[DragBar] Showing bar');
    ...
    hide() {
        ...
        console.log('[DragBar] Hiding bar');
     ...
     */
    clear() {
        console.log('[DragBar] Clearing selection');
        this.selectedFiles = [];
        this.hide();
    },
     ...
     */
    destroy() {
        console.log('[DragBar] Destroying...');
