...
const NodusChains = {
  ...
  async createChain(name, color = '#facc15') {
    ...
    console.log('[Chains] Created:', newChain);
    return newChain;
  },
  ...
  async updateChainColor(chainId, newColor) {
    ...
    chain.updated_at = new Date().toISOString();
    await this._saveChains(chains);
    console.log('[Chains] Updated color:', chainId, newColor);
    return chain;
  },
  ...
  async updateChainNotes(chainId, notes) {
    ...
    console.log('[Chains] Notes updated:', chainId);
    return chain;
  },
  ...
  async addChainAttachment(chainId, fileName, fileSize) {
    ...
    console.log('[Chains] Attachment added:', fileName);
    return chain;
  },
  ...
  async removeChainAttachment(chainId, fileName) {
    ...
    if (index !== -1) {
      ...
      if (window.NodusAttachmentsDB) {
        try {
          ...
          console.log('[Chains] Attachment file deleted from IndexedDB:', dbKey);
        } catch (err) {
    ...
    console.log('[Chains] Attachment removed:', fileName);
    return chain;
  }
};
