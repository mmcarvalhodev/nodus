...
console.log('%c📦 NODUS Modules v3.0', 'color: #3b82f6; font-weight: bold;');
...
window.NodusDebug = {
  ...
  log(level, module, message, data = null) {
    ...
    if (data !== null) {
      console.log(`%c${prefix} ${message}`, levelConfig.style, data);
    } else {
      console.log(`%c${prefix} ${message}`, levelConfig.style);
    }
  },
  ...
  banner(message) {
    ...
    console.log(`%c${line}\n${message}\n${line}`, 'color: #f59e0b; font-weight: bold; font-size: 16px');
  },
...
console.log('%c✅ NodusDebug carregado', 'color: #10b981; font-weight: bold;');
...
window.NodusStorage = {
  ...
  async saveIdea(idea) {
    ...
    try {
      ...
      // ✅ Verificar permissão pela API canônica (server-authoritative).
      // canUseQueue() consulta capabilities do server — nada hardcoded aqui.
      if (window.NodusLicense && !window.NodusLicense.canUseQueue(queueKey)) {
          NodusDebug.warn('Storage', `⚠️ Queue bloqueada por gate: ${queueKey}`);
          NodusDebug.groupEnd();
          NodusDebug.timeEnd('Storage.saveIdea');

          // Mostrar paywall (mesmo mapping que antes — UI fica)
          const featureMap = {
          ...
          if (window.NodusLicense.showPaywall) {
            window.NodusLicense.showPaywall(featureMap[queueKey] || 'queue_locked');
          }
...
window.NodusPanelNQ = {
  ...
  tagColors:  {}, // Mapa tag→cor persistido no storage (compartilhado com dashboard_cards)
  customTags: [], // Tags criadas pelo utilizador (persistidas em nodus_custom_tags)
  ...
  async open(ideaData) {
    ...
    try {
      ...
      // Carregar cores e tags customizadas persistidas do storage
      const stored = await chrome.storage.local.get(['nodus_tag_colors', 'nodus_custom_tags']);
      this.tagColors  = stored.nodus_tag_colors  || {};
      this.customTags = stored.nodus_custom_tags || [];
      ...
      this.currentTags = (ideaData.tags || []).map(tagName => {
        ...
        const color = suggestedTag
          ? suggestedTag.color
          : (this.tagColors[upperTag] || '#6b7280');
        return {
          name: tagName,
  ...
  renderSuggestedTags() {
    ...
    // Tags customizadas criadas pelo utilizador (sem bolinha LED, com long-press para excluir)
    const suggestedNames = new Set(this.SUGGESTED_TAGS.map(t => t.name.toLowerCase()));
    const customOnly = this.customTags.filter(t => !suggestedNames.has(t.toLowerCase()));

    if (customOnly.length > 0) {
      customOnly.forEach(tagName => {
        const upperTag = tagName.toUpperCase();
        const color = this.tagColors[upperTag] || '#6b7280';
        ...
        btn.addEventListener('mousedown', (e) => {
          e.preventDefault();
          pressTimer = setTimeout(enterDeleteMode, 700);
        });
        ...
        btn.addEventListener('mouseenter', () => {
          if (!deleteMode) {
            btn.style.transform = 'scale(1.05)';
            btn.style.boxShadow = `0 0 8px ${color}`;
          }
        });
        ...
        btn.addEventListener('click', () => {
          if (deleteMode) {
            exitDeleteMode();
            this._deleteCustomTag(tagName);
          } else {
            this.addTag(tagName.toLowerCase(), this.defaultQueue, color);
          }
        });
  ...
  addTag(tagName, queueKey = 'ideas_queue_default', tagColor = null) {
    ...
    // Se não passou cor, tenta encontrar nos SUGGESTED_TAGS → depois no storage → depois atribui nova
    if (!tagColor) {
      const upperTag = tagName.toUpperCase();
      const suggestedTag = this.SUGGESTED_TAGS.find(t => t.name.toLowerCase() === tagName);
      if (suggestedTag) {
        tagColor = suggestedTag.color;
      } else if (this.tagColors[upperTag]) {
        tagColor = this.tagColors[upperTag]; // Cor já persistida
      } else {
        tagColor = this._assignTagColorSync(tagName); // Atribui nova cor e persiste
      }
    }

    // Registar tag customizada em nodus_custom_tags se não for sugerida
    const isSuggested = this.SUGGESTED_TAGS.some(t => t.name.toLowerCase() === tagName);
    if (!isSuggested && !this.customTags.includes(tagName)) {
      this.customTags.push(tagName);
      chrome.storage.local.set({ nodus_custom_tags: this.customTags });
    }
  ...
  // ─────────────────────────────────────────────────────────────
  // ATRIBUIR COR ALEATÓRIA A TAG CUSTOMIZADA (síncrono + persiste async)
  // ─────────────────────────────────────────────────────────────
  _assignTagColorSync(tagName) {
    const upperTag = tagName.toUpperCase();
    ...
    // Persiste de forma assíncrona (não bloqueia a UI)
    chrome.storage.local.set({ nodus_tag_colors: this.tagColors });
    return color;
  },

  // ─────────────────────────────────────────────────────────────
  // EXCLUIR TAG CUSTOMIZADA DO SISTEMA (long-press)
  // ─────────────────────────────────────────────────────────────
  _deleteCustomTag(tagName) {
    const lowerTag = tagName.toLowerCase();
    const upperTag = tagName.toUpperCase();

    // Remove da lista de tags customizadas
    this.customTags = this.customTags.filter(t => t.toLowerCase() !== lowerTag);

    // Remove a cor persistida
    delete this.tagColors[upperTag];

    // Persiste ambos
    chrome.storage.local.set({
      nodus_custom_tags: this.customTags,
      nodus_tag_colors: this.tagColors,
    });

    // Se a tag estava seleccionada no card actual, remove-a também
    this.removeTag(tagName.toLowerCase());

    // Re-renderiza a área de sugestões para reflectir a remoção
    this.renderSuggestedTags();

    NodusDebug.info('PanelNQ', `Tag excluída: ${tagName}`);
  },

  // ─────────────────────────────────────────────────────────────
  // REMOVER TAG (do card actual, não do sistema)
  // ─────────────────────────────────────────────────────────────
  removeTag(tagName) {
...
const NodusGate = {
  ...
  showWelcomeModal() {
    console.log('[Gate] Showing Ethical Gate...');
  ...
  async checkAcceptanceAndProceed(callback) {
    console.log('[Gate] Checking acceptance...');
    const accepted = await this.hasAcceptedTerms();
    
    if (accepted) {
      console.log('[Gate] ✅ Acceptance OK');
      callback();
    } else {
      console.log('[Gate] ⚠️ Showing modal');
...
console.log('%c🛡️ Ethical Gate v3.0 loaded', 'color: #60a5fa; font-weight: bold;');
...
console.log('%c✅ NODUS_UI exportado e pronto', 'color: #10b981; font-weight: bold;');
...
window.NODUS_TEST_CHANGE_QUEUE = (tagName, queueKey) => {
  console.log(`🧪 TESTE: Mudando tag "${tagName}" para fila "${queueKey}"`);
  ...
  console.log(`   Index encontrado: ${tagIndex}`);
  console.log(`   currentTags:`, JSON.parse(JSON.stringify(panel.currentTags)));
  
  if (tagIndex !== -1) {
    const oldQueue = panel.currentTags[tagIndex].queueKey;
    panel.currentTags[tagIndex].queueKey = queueKey;
    console.log(`   ✅ Alterado de ${oldQueue} para ${queueKey}`);
    panel.renderTags();
    panel.updateQueuePreview();
    console.log(`   currentTags após:`, JSON.parse(JSON.stringify(panel.currentTags)));
  } else {
    console.error(`   ❌ Tag não encontrada!`);
  }
};

console.log("🧪 Função de teste disponível: NODUS_TEST_CHANGE_QUEUE(tagName, queueKey)");
console.log("   Exemplo: NODUS_TEST_CHANGE_QUEUE(\"chatgpt\", \"ideas_queue_custom1\")");
...
const initTestFunctions = () => {
  ...
  window.NODUS_LICENSE_STATUS = () => {
    if (window.NodusLicense) {
      console.log('Status:', window.NodusLicense.license);
      console.log('É PRO?', window.NodusLicense.isPro());
  ...
  window.NODUS_SET_FREE = async () => {
    if (window.NodusLicense) {
      ...
      console.log('Revertido para FREE');
    }
  };

  console.log("🧪 Funções de licenciamento disponíveis:");
  console.log("   NODUS_TEST_PAYWALL('export_html') - Mostra paywall");
  console.log("   NODUS_TEST_ACTIVATE() - Mostra modal de ativação");
  console.log("   NODUS_LICENSE_STATUS() - Mostra status atual");
  console.log("   NODUS_SET_FREE() - Reverte para FREE");
  
  // Função de debug de telemetria
  window.NODUS_DEBUG_TELEMETRY = async () => {
    console.log('🔍 ===== DEBUG TELEMETRIA NODUS =====');
    ...
    console.log('   telemetryMode:', settings.settings?.telemetryMode);
    console.log('');
    console.log('📝 Event Log:');
    console.log('   Total events:', eventLog.telemetry_event_log?.length || 0);
    ...
    if (eventLog.telemetry_event_log && eventLog.telemetry_event_log.length > 0) {
      ...
      eventLog.telemetry_event_log.slice(0, 10).forEach((event, i) => {
        console.log(`   ${i+1}. [${event.event_type}] @ ${new Date(event.timestamp).toLocaleString()}`);
        console.log('      ', event);
      });
    } else {
    ...
    console.log('');
    console.log('📈 Stats:', stats.telemetry_stats);
  ...
  console.log("   NODUS_DEBUG_TELEMETRY() - Debug de telemetria");
};
