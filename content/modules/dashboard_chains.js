...
(async () => {
  try {
    ...
    console.log('[Chains UI] ✅ Telemetry loaded');
  } catch (error) {
...
const NodusChainsUI = {
  ...
  async init() {
    ...
    if (!document._licenseChangeListenerAttached) {
      window.addEventListener('nodus-license-changed', async (e) => {
        console.log('[Chains UI] 🔄 Licença mudou para:', e.detail.status);
  ...
  setupStateSyncListener() {
    ...
    console.log('[Chains UI] 🔄 Configurando sincronização de estado...');
    
    // Sincronizar quando chains mudarem
    window.NodusStateSync.subscribe('nodus_chains', async (newValue) => {
      console.log('[Chains UI] 🔄 Chains atualizadas em outra tab');
      await this.render();
      console.log('[Chains UI] ✅ Chains recarregadas');
    });
    ...
    ['ideas_queue_quick', 'ideas_queue_default', 'ideas_queue_automatic', 
     'ideas_queue_q1', 'ideas_queue_custom2', 'ideas_queue_custom3', 'ideas_queue_custom4'].forEach(queueKey => {
      window.NodusStateSync.subscribe(queueKey, async (newValue) => {
        console.log(`[Chains UI] 🔄 Queue ${queueKey} atualizada em outra tab`);
        await this.loadAllIdeas();
    ...
    console.log('[Chains UI] ✅ Sincronização de estado ativa!');
  },
  
  async loadAllIdeas() {
    console.log('[Chains UI] Carregando todas as ideias...');
    ...
    console.log('[Chains UI] Total ideias carregadas:', this.allIdeas.length);
  },
  
  attachEventsGlobal() {
    console.log('[Chains UI] Anexando eventos globais');
    
    // Input de busca (delegado)
    document.addEventListener('input', (e) => {
      if (e.target.id === 'sidebarSearch') {
        this.searchQuery = e.target.value.toLowerCase();
        console.log('[Chains UI] 🔍 Busca:', this.searchQuery);
        this.render();
      }
    });
    ...
    document.addEventListener('click', async (e) => {
      ...
      if (target.id === 'newChainBtn' || target.id === 'newChainWelcome') {
        e.preventDefault();
        e.stopPropagation();
        console.log('✅ [Chains] Nova chain - mostrar input');
      ...
      if (target.id === 'addNodeBtn') {
        e.preventDefault();
        e.stopPropagation();
        console.log('✅ [Chains] Add Node - abrir sidebar');
      ...
      if (target.id === 'toggleFileTrayBtn' || target.closest('#toggleFileTrayBtn')) {
        e.preventDefault();
        e.stopPropagation();
        console.log('✅ [Chains] Toggle File Tray');
      ...
      if (target.id === 'fullChatBtn') {
        e.preventDefault();
        e.stopPropagation();
        console.log('✅ [Chains] Full Chat - iniciar captura');
      ...
      if (target.classList.contains('chain-name-edit')) {
        e.preventDefault();
        e.stopPropagation();
        console.log('✅ [Chains] Renomear chain - mostrar input');
      ...
      if (target.classList.contains('chain-color-btn')) {
        ...
        console.log('✅ [Chains] Cor selecionada:', newColor);
        return;
      }
      ...
      if (target.classList.contains('chain-action-delete') || target.closest('.chain-action-delete')) {
        ...
        const chainId = btn.dataset.chainId;
        console.log('[Chains] 🗑️ Botão Delete clicado, chainId:', chainId);
        document.querySelectorAll('.chain-actions-menu').forEach(m => m.classList.remove('show'));
      ...
      if (target.classList.contains('chain-action-attachments') || target.closest('.chain-action-attachments')) {
        ...
        const chainId = btn.dataset.chainId;
        console.log('[Chains] 📎 Botão Attachments clicado, chainId:', chainId);
        document.querySelectorAll('.chain-actions-menu, .chain-actions-menu-header').forEach(m => m.style.display = 'none');
      ...
      if (target.classList.contains('chain-action-notes') || target.closest('.chain-action-notes')) {
        ...
        const chainId = btn.dataset.chainId;
        console.log('[Chains] 📝 Botão Notes clicado, chainId:', chainId);
        document.querySelectorAll('.chain-actions-menu, .chain-actions-menu-header').forEach(m => m.style.display = 'none');
      ...
      if (target.id === 'openSidebarBtn') {
        e.preventDefault();
        e.stopPropagation();
        console.log('✅ [Chains] Toggle sidebar - ANTES:', this.sidebarOpen);
        this.sidebarOpen = !this.sidebarOpen;
        console.log('✅ [Chains] Toggle sidebar - DEPOIS:', this.sidebarOpen);
        await this.render();
      ...
      console.log('🔍 [Chains] Verificando sidebar-card:', !!sidebarCard);
      
      if (sidebarCard) {
        e.preventDefault();
        e.stopPropagation();
        const ideaId = sidebarCard.dataset.ideaId;
        console.log('✅ [Chains] Card clicado! ID:', ideaId, 'Selecionados antes:', this.selectedIdeaIds.size);
        
        if (this.selectedIdeaIds.has(ideaId)) {
          this.selectedIdeaIds.delete(ideaId);
          console.log('➖ [Chains] Removido da seleção');
        } else {
          this.selectedIdeaIds.add(ideaId);
          console.log('➕ [Chains] Adicionado à seleção');
        }
        
        console.log('🔄 [Chains] Total selecionados agora:', this.selectedIdeaIds.size);
        await this.render();
        return;
      }
      
      // Queue navigation
      console.log('[DEBUG QUEUE NAV] Target ID:', target.id, 'Current Index:', this.currentQueueIndex);
      ...
      if (target.id === 'prevQueueBtn') {
        ...
        if (this.currentQueueIndex > 0) {
          ...
          console.log('✅ [Chains] Fila anterior:', this.currentQueueIndex);
          await this.render();
        }
        return;
      }
      ...
      if (target.id === 'nextQueueBtn') {
        ...
        console.log('[DEBUG] Current index:', this.currentQueueIndex);
        console.log('[DEBUG] Max index: 6');
        
        if (this.currentQueueIndex < 6) {
          this.currentQueueIndex++;
          console.log('✅ [Chains] Próxima fila:', this.currentQueueIndex);
          await this.render();
        } else {
          console.log('⚠️ [Chains] Já está na última fila');
        }
        return;
      }
      ...
      if (target.classList.contains('sidebar-grid-btn')) {
        ...
        console.log('✅ [Chains] Sidebar grid:', this.sidebarGrid);
        await this.render();
      ...
      if (target.classList.contains('idea-checkbox')) {
        e.stopPropagation();
        console.log('✅ [Chains] Checkbox toggled');
      ...
      */
      ...
      if (target.id === 'addSelectedBtn') {
        e.preventDefault();
        e.stopPropagation();
        console.log('✅ [Chains] Adicionar selecionados');
      ...
      if (target.id === 'clearSelectionBtn') {
        e.preventDefault();
        e.stopPropagation();
        console.log('✅ [Chains] Limpar seleção');
        this.selectedIdeaIds.clear();
        await this.render();
        return;
      }
      ...
      if (target.classList.contains('grid-btn')) {
        e.preventDefault();
        e.stopPropagation();
        console.log('✅ [Chains] Grid mudado');
        this.viewMode = 'grid';
      ...
      if (target.id === 'graphModeBtn') {
        e.preventDefault();
        e.stopPropagation();
        console.log('✅ [Chains] Mudou para Graph Mode');
        this.viewMode = 'graph';
      ...
      if (target.id === 'expandGraphBtn') {
        ...
        if (this.viewMode === 'graph') {
          console.log('✅ [Chains] Toggle Expand Graph');
      ...
      if (target.classList.contains('chain-tab')) {
        ...
        if (this.currentGrid > 1 && this.viewMode === 'grid') {
          ...
          console.log('[Chains] Grid II/III - Ajustado para início do bloco:', blockStart);
        } else {
      ...
      if (target.classList.contains('toggle-btn')) {
        e.preventDefault();
        e.stopPropagation();
        console.log('✅ [Chains] Toggle display');
        const ideaId = target.dataset.ideaId;
        const chainId = target.dataset.chainId;
      ...
      if (target.classList.contains('remove-node-btn')) {
        ...
        const chainId = target.dataset.chainId;
        console.log('✅ [Chains] Remover node:', { nodeId, chainId });
      ...
      if (target.classList.contains('promote-node-btn')) {
        ...
        e.stopPropagation();
        const ideaId = target.dataset.ideaId;
        console.log('✅ [Chains] Promover node:', ideaId);
  ...
  async addSelectedToChain() {
    ...
    console.log('[Chains UI] Adicionados', this.selectedIdeaIds.size, 'nós');
  ...
  async render(providedChains = null) {
    console.log('[Chains UI] 🎨 Render iniciado.');
    console.log('[Chains UI] 🔍 this.sidebarOpen =', this.sidebarOpen);
    console.log('[Chains UI] 🔍 this.currentChainIndex =', this.currentChainIndex);
    console.log('[Chains UI] 🔍 providedChains:', providedChains ? providedChains.length : 'null');
    console.log('[Chains UI] 🔑 License isPro():', window.NodusLicense?.isPro(), '- license:', window.NodusLicense?.license);
    
    // BUSCAR CHAINS (usar providedChains se disponível para evitar race condition)
    console.log('[Chains UI] 📡 Chamando getAllChains()...');
    ...
    console.log('[Chains UI] 📊 Total chains (filtradas):', chains.length);
    ...
    if (chains.length > 0 && this.currentChainIndex >= chains.length) {
      ...
      console.log(`[Chains UI] ⚠️ Índice ${oldIndex} → ${this.currentChainIndex} (total: ${chains.length})`);
    }
    ...
    console.log('[Chains UI] 📊 Chain atual (index', this.currentChainIndex, '):', chains[this.currentChainIndex]);
    ...
    if (!document.getElementById('globalGraphHoverCard')) {
      console.log('[Chains UI] 🎨 Criando hover card global no overlay...');
      ...
      console.log('[Chains UI] ✅ Hover card criado no overlay!');
    }
    
    console.log('[Chains UI] 📊 Chains:', chains.length, '| Sidebar:', this.sidebarOpen ? 'ABERTA' : 'FECHADA');
    ...
    // When leaving graph mode, always hide the hover card so it doesn't ghost
    // over grid-mode content (its background is identical to the modal, making it
    // look like content is clipped on the left).
    if (this.viewMode !== 'graph') {
      const hc = document.getElementById('globalGraphHoverCard');
    ...
    sidebar.innerHTML = `
      ...
      <div class="sidebar-queue-selector">
        ${(() => {
          console.log('[RENDER DEBUG] currentQueueIndex:', this.currentQueueIndex, 'tipo:', typeof this.currentQueueIndex);
          console.log('[RENDER DEBUG] >= 6?', this.currentQueueIndex >= 6);
          return '';
        })()}
    ...
    console.log('[Chains UI] ✅ innerHTML setado. Sidebar width deveria ser:', this.sidebarOpen ? (this.sidebarGrid === 1 ? '320px' : '640px') : '0');
    
    if (chains.length > 0) {
      await this.renderNodes();
    }
    
    console.log('[Chains UI] 🏁 Render concluído');
  ...
  async handleInlineChainNameConfirm() {
    ...
    if (this.isFullChatMode) {
      console.log('[FullChat] Modo Full Chat detectado, processando...');
  ...
  initDragAndDrop() {
    ...
    console.log('[Chains UI] 🎯 Inicializando drag & drop -', sidebarCards.length, 'sidebar cards,', nodeCards.length, 'node cards,', graphNodes.length, 'graph nodes');
  ...
  handleSidebarDragStart(e, card) {
    ...
    console.log('[Chains UI] 🎯 Sidebar drag start:', ideaId);
  ...
  handleNodeDragStart(e, card) {
    ...
    console.log('[Chains UI] 🎯 Node drag start:', this.draggedNodeId);
  ...
  handleDragEnd(e) {
    console.log('[Chains UI] 🎯 Drag end');
  ...
  async handleChainDrop(e) {
    ...
    if (!this.draggedFromSidebar && this.draggedNodeId) {
      console.log('[Chains UI] 🎯 Reordenando nós...');
      ...
      console.log('[Chains UI] Nova ordem:', newOrder);
    ...
    if (this.draggedFromSidebar) {
      try {
        ...
        console.log('[Chains UI] 🎯 Drop -', draggedIds.length, 'ideias');
        ...
        if (this.currentGrid > 1 && this.viewMode === 'grid') {
          ...
          if (chainColumn && chainColumn.dataset.chainIndex) {
            ...
            console.log('[Chains UI] ✅ Drop detectado via .chain-column:', targetChainIndex);
          } else {
            ...
            console.log('[Chains UI] ⚠️ Drop detectado via cálculo (fallback):', {
              relativeX,
              columnWidth,
              columnIndex,
              targetChainIndex
          ...
          if (chainElement && chainElement.dataset.chainIndex !== undefined) {
            ...
            console.log('[Chains UI] ✅ Drop em Graph Mode detectado via data-chain-index:', targetChainIndex);
          } else {
            ...
            console.log('[Chains UI] ⚠️ Drop em Graph Mode via cálculo (fallback):', {
              mouseX: this.lastDragPosition.x,
        ...
        console.log('[Chains UI] 🎯 Adicionando à chain:', targetChainIndex, 'ID:', chainId);
        console.log('[Chains UI] 📋 Todas as chains:', chains.map((c, i) => `[${i}] ${c.name} (${c.id})`));
        console.log('[Chains UI] 🎯 Chain selecionada:', chains[targetChainIndex].name);
        ...
        console.log('[Chains UI] ✅ Drop concluído!');
      } catch (err) {
  ...
  renderSidebarIdeas() {
    console.log('[Chains UI] 📋 renderSidebarIdeas - Total:', this.allIdeas.length);
    ...
    const nonVirtualIdeas = this.allIdeas.filter(idea => {
      ...
      if (isVirtual) {
        console.log('[Chains UI] 🚫 Filtrando idea virtual:', idea.id, idea.queue);
      }
      return !isVirtual;
    });
    
    console.log('[Chains UI] Ideas após filtrar virtuais:', nonVirtualIdeas.length);
  ...
  async renderNodes() {
    ...
    console.log('[Chains] renderNodes - currentGrid:', this.currentGrid, 'viewMode:', this.viewMode, 'chains:', chains.length);

    // Defensive reset: restores grid-mode defaults before each render.
    // Needed because renderNodes() is called directly (drag-and-drop, display toggle, etc.)
    // without a full render() rebuilding the HTML. Graph mode sets display:block on nodesGrid;
    // without this reset, switching graph → grid I leaves the node cards left-collapsed.
    if (this.viewMode !== 'graph') {
      nodesGrid.style.display = 'flex';
      nodesGrid.style.flexDirection = 'row';
      nodesGrid.style.justifyContent = 'center';
      nodesGrid.style.alignItems = '';
      nodesGrid.style.gridTemplateColumns = '';
      nodesGrid.style.gap = '';
      nodesGrid.style.justifyItems = '';
      nodesGrid.style.alignContent = '';
      nodesGrid.style.padding = '';
      nodesGrid.style.overflowX = '';
      nodesGrid.style.height = '';
      nodesGrid.style.position = '';
      nodesGrid.style.flex = '1';
      nodesGrid.style.width = '100%';
      nodesGrid.style.overflowY = 'scroll';

      // Hide the graph hover card. If the user was hovering a graph node when switching
      // to View I/II/III, the mouseleave event never fires on the removed element and the
      // card stays display:block — its dark background (matching the modal) makes View I
      // content appear clipped on the left. Force-hide it on every non-graph render.
      const graphHoverCard = document.getElementById('globalGraphHoverCard');
      if (graphHoverCard) graphHoverCard.style.display = 'none';
    }

    // Graph Mode: SEMPRE renderizar todas as chains horizontalmente
    if (this.viewMode === 'graph') {
      console.log('[Chains] ✅ GRAPH MODE - Renderizando TODAS as chains horizontalmente');
    ...
    if (this.currentGrid > 1 && this.viewMode === 'grid') {
      if (chains.length > 1) {
        console.log('[Chains] ✅ Renderizando MÚLTIPLAS chains');
        await this.renderMultipleChains(chains, nodesGrid);
        return;
      } else {
        console.log('[Chains] ⚠️ Só tem 1 chain, renderizando modo normal');
  ...
  async renderGraphMode(chain, nodesGrid) {
    console.log('[Chains UI] 🎨 Renderizando GRAPH MODE');
  ...
  async renderGraphModeAllChains(allChains, nodesGrid) {
    console.log('[Chains UI] 🎨 Renderizando GRAPH MODE - TODAS as chains horizontalmente');
  ...
  async renderGridMode(chain, nodesGrid) {
    console.log('[Chains UI] 🎨 Renderizando GRID MODE (chain única) - nodes:', chain.nodes.length);
    
    // Cor da chain (padrão amarelo se não definida)
    const chainColor = chain.color || '#facc15';
    
    // Renderizar nodes com linhas conectoras - LARGURA FIXA COMO CARDS
    let html = `<div style="display:flex; flex-direction:column; align-items:center; width:100%; padding:15px; padding-bottom:100px; box-sizing:border-box;">
    ...
    if (this.justCapturedFullChat) {
      console.log('[Chains UI] 🎬 Primeira renderização após Full Chat - ANIMANDO!');
      this.justCapturedFullChat = false; // Resetar flag
      
      requestAnimationFrame(() => {
        const cards = nodesGrid.querySelectorAll('.node-card');
        console.log('[Chains UI] 🎬 Animando', cards.length, 'cards com aceleração progressiva');
        ...
        console.log('[Chains UI] 📊 Delays:', delays.slice(0, 3), '...', delays.slice(-2));
      ...
      console.log('[Chains UI] 👁️ Visualização normal - sem animação');
      requestAnimationFrame(() => {
        const cards = nodesGrid.querySelectorAll('.node-card');
        cards.forEach(card => {
          card.style.opacity = '1';
  ...
  async renderMultipleChains(allChains, nodesGrid) {
    console.log('[Chains UI] 🎨 Renderizando MÚLTIPLAS CHAINS (Grid II/III)');
    ...
    // APENAS na primeira vez após Full Chat Capture
    if (this.justCapturedFullChat) {
      console.log('[Chains UI] 🎬 Primeira renderização após Full Chat (Grid II/III) - ANIMANDO!');
      
      requestAnimationFrame(() => {
        const cards = nodesGrid.querySelectorAll('.node-card');
        console.log('[Chains UI] 🎬 Animando', cards.length, 'cards (Grid II/III) com aceleração progressiva');
        ...
        console.log('[Chains UI] 📊 Delays (Grid II/III):', delays.slice(0, 3), '...', delays.slice(-2));
      ...
      // Visualização normal: mostrar cards instantaneamente
      console.log('[Chains UI] 👁️ Visualização normal (Grid II/III) - sem animação');
      requestAnimationFrame(() => {
        const cards = nodesGrid.querySelectorAll('.node-card');
        cards.forEach(card => {
          card.style.opacity = '1';
  ...
  applyPlatformSpecificMargin(marginValue) {
    ...
    if (hostname.includes('claude.ai')) {
      console.log('[Chains] 🔍 Aplicando margin no Claude:', marginValue + 'px');
      ...
      for (let i = 0; i < strategies.length; i++) {
        try {
          ...
          if (element && element.offsetHeight > 100) {
            ...
            console.log(`[Chains] ✅ Margin Claude aplicado (estratégia ${i + 1}):`, element.tagName, marginValue + 'px');
            return;
          }
        } catch (e) {
    ...
    // CHATGPT
      ...
      console.log('[Chains] 🔍 Aplicando margin no ChatGPT:', marginValue + 'px');
      ...
      if (main) {
        ...
        if (!main.dataset.originalMarginRight) {
          ...
          console.log('[Chains] 📝 Margin original do ChatGPT:', currentMargin);
        }
        ...
        console.log('[Chains] ✅ Margin ChatGPT aplicado no <main>:', marginValue + 'px');
        return;
      }
      ...
      console.log('[Chains] ℹ️ Plataforma não suportada para margin, usando CSS normal');
    }
  },
  ...
  removePlatformSpecificMargin() {
    ...
    if (elementsWithMargin.length > 0) {
      console.log('[Chains] 🧹 Removendo margin');
      elementsWithMargin.forEach(el => {
        ...
        if ((hostname.includes('chatgpt.com') || hostname.includes('chat.openai.com')) && 
          ...
          console.log('[Chains] ↩️ Margin original do ChatGPT restaurado:', el.dataset.originalMarginRight);
  ...
  async copyChain(chainId) {
    console.log('[Chains] 📋 Copiando chain:', chainId);
  ...
  async exportChainTXT(chainId) {
    console.log('[Chains] 📄 Exportando chain TXT:', chainId);
  ...
  async exportChainMD(chainId) {
    console.log('[Chains] 📝 Exportando chain MD:', chainId);
  ...
  showExportSubmenu(buttonElement, chainId) {
    ...
    console.log('[Export] isPro:', isPro, 'license:', window.NodusLicense?.license);
  ...
  async exportChainHTML(chainId) {
    console.log('[Chains] 🌐 Exportando chain HTML:', chainId);
...
<body>
  ...
  async exportChainDOCX(chainId) {
    console.log('[Chains] 📘 Exportando chain DOC (Word):', chainId);
...
<body>
  ...
  async injectChain(chainId) {
    console.log('[Chains] 🚀 Injetando chain:', chainId);
  ...
  async injectAsText(chain, platform, analysis) {
    console.log('[Chains] ⚡ Injetando como texto estruturado');
  ...
  async injectAsMarkdown(chain, platform) {
    console.log('[Chains] 📎 Injetando como Markdown');
    ...
    // Gemini não expõe input[type=file] acessível via DOM — o '+' abre um menu
    // que termina num file picker nativo do SO. Também não existe um botão
    // confiável para anexar "de fora" (as tentativas de auto-clicar o '+'
    // acabaram abrindo o menu da resposta do assistente). Caminho correto:
    // expor o chip arrastável e deixar o Gemini reconhecer o drag nativamente
    // (o bridge MAIN-world popula items.add(file) + DownloadURL no dragstart).
    if (platform === 'gemini') {
      this._showDraggableFilePanel(file, blob, platform, markdown);
      return;
    }

    // Outras plataformas: tenta upload direto via input[type=file] e cai para
    // o painel se não achar.
  ...
  /**
   * Não-op: o bridge MAIN world agora é carregado como content script via manifest.json
   * (content/modules/chains_drag_bridge.js, world: MAIN).
   * Mantido para evitar erros em chamadas legadas; pode ser removido futuramente.
   */
  _injectMainWorldDragBridge() {
    // Bridge já carregado pelo manifest — nada a fazer aqui
  },

  /**
   * Mostra painel flutuante com opções para enviar arquivo ao chat.
   * Para Gemini: exibe chip arrastável que replica o padrão do NodusAttachmentsDragBar
   * (que já funciona para imagens) — sem download, sem drop sintético.
   */
  _showDraggableFilePanel(file, blob, platform, markdownContent) {
    ...
    // Cria URL de objeto para drag (revogada ao fechar painel)
    const blobUrl = URL.createObjectURL(blob);
    // Isolated-world URL usada no setData('DownloadURL') do chip (Gemini)
    let isolatedBlobUrl = null;
    ...
    // Bloco de drag chip — só exibido para Gemini (e qualquer plataforma sem suporte a upload)
    const geminiDragBlock = isGemini ? `
      <div id="nodus-drag-chip" draggable="true" style="
        display:flex; align-items:center; gap:10px;
        background:#1e2535; border:2px dashed #a78bfa;
      ...
      ">
        ...
        <div style="flex:1; min-width:0;">
          <div style="color:#e2e8f0; font-size:12px; font-weight:600; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${this.escapeHtml(file.name)}</div>
          <div style="color:#6b7280; font-size:11px; margin-top:2px;">${sizeStr} · Markdown</div>
        </div>
        <div style="text-align:center; flex-shrink:0;">
          <div style="font-size:18px;">✋</div>
          <div style="color:#a78bfa; font-size:9px; font-weight:700; margin-top:1px;">ARRASTE</div>
        </div>
      </div>
      <div style="color:#6b7280; font-size:11px; text-align:center; margin-bottom:10px; line-height:1.4;">
        Arraste o chip acima sobre a caixa de mensagem do Gemini e solte. O arquivo será anexado igual quando você escolhe pelo botão "+".
      </div>
    ` : '';

    // Botão "Injetar no chat":
    // - Não-Gemini: tenta via input[type=file] (uploadFileToplatform)
    // - Gemini: o chip arrastável acima é a via — botão não é renderizado
    //   (alinha o UX com o NodusAttachmentsDragBar, que também é só drag).
    const injectBtn = isGemini ? '' : `
    ...
    panel.innerHTML = `
      <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:10px;">
        <span style="color:#a78bfa; font-size:12px; font-weight:700;">📎 Injetar como arquivo</span>
    ...
    const close = () => {
      // Limpa o arquivo do MAIN world bridge e revoga blob URLs
      window.postMessage({ __nodus: 'cdrag', t: 'clr' }, location.origin);
      URL.revokeObjectURL(blobUrl);
      if (isolatedBlobUrl) { URL.revokeObjectURL(isolatedBlobUrl); isolatedBlobUrl = null; }
      panel.remove();
    };
    ...
    // ✋ Drag chip (Gemini)
    // Padrão espelhado do NodusAttachmentsDragBar (que funciona para imagens no Gemini):
    //   1. File realista com lastModified randomizado nas últimas 24h
    //   2. dragstart no mundo isolado chama items.add(file) diretamente
    //   3. MAIN world bridge (chains_drag_bridge.js) roda em fase capture e repete
    //      items.add — redundância de segurança caso o Gemini filtre o DataTransfer
    //      do mundo isolado para arquivos não-imagem.
    if (isGemini) {
      // Recria o File com lastModified realista (mesmo truque usado pelo DragBar)
      const realisticFile = new File([blob], file.name, {
        type: file.type || 'text/markdown',
        lastModified: Date.now() - Math.floor(Math.random() * 86400000)
      });

      const chip = document.getElementById('nodus-drag-chip');
      chip.addEventListener('dragstart', (e) => {
        console.log('[Chains] Drag chip — dragstart (isolated world)');

        e.dataTransfer.effectAllowed = 'copy';

        // Estratégia comprovada (espelhada do NodusAttachmentsDragBar):
        //   Custom MIME type 'application/x-nodus-chain-md' que persiste no
        //   drop (diferente do 'Files'/'DownloadURL' que o Chrome remove em
        //   drags iniciados por extensão — confirmado por console logs).
        //   O NodusDropHandler vai pegar esse tipo no drop, reconstruir o
        //   File a partir do payload, e preencher o input[type=file] do
        //   Gemini diretamente — mesmo caminho que já funciona para imagens.
        try {
          e.dataTransfer.setData('application/x-nodus-chain-md', JSON.stringify({
            fileName: file.name,
            content: markdownContent,
            source: 'nodus-chain'
          }));
          console.log('[Chains] setData application/x-nodus-chain-md ✅');
        } catch (err) {
          console.warn('[Chains] setData nodus-chain-md failed:', err);
        }

        // Best-effort: items.add (geralmente stripped pelo Chrome mas harmless)
        try {
          e.dataTransfer.items.add(realisticFile);
        } catch (_) {}

        console.log('[Chains] types após isolated dragstart:', JSON.stringify(Array.from(e.dataTransfer.types)));

        // Efeitos visuais + remover blur do overlay (alinha com DragBar)
        document.body.classList.add('nodus-dragging');
        const overlay = document.getElementById('nodus-dashboard-overlay');
        if (overlay) overlay.classList.add('dragging');
        chip.style.borderColor = '#10b981';
        chip.style.background = '#0d1f17';
        chip.style.cursor = 'grabbing';
      });
      chip.addEventListener('dragend', () => {
        console.log('[Chains] Drag chip — dragend');
        document.body.classList.remove('nodus-dragging');
        const overlay = document.getElementById('nodus-dashboard-overlay');
        if (overlay) overlay.classList.remove('dragging');
        chip.style.borderColor = '#a78bfa';
        chip.style.background = '#1e2535';
        chip.style.cursor = 'grab';
      });
    }

    // 🚀 Injetar no chat (não renderizado para Gemini — chip arrastável é a via)
    // Plataformas com input[type=file] acessível: uploadFileToplatform
    const injectBtnEl = document.getElementById('nodus-panel-btn-inject');
    if (injectBtnEl) {
      injectBtnEl.onclick = async () => {
        const btn = document.getElementById('nodus-panel-btn-inject');
        btn.disabled = true;
        btn.textContent = '⏳ Procurando input...';
        const detectedPlatform = platform || this.detectCurrentPlatform() || 'chatgpt';
        const success = await this.uploadFileToplatform(file, detectedPlatform);
        if (success) {
    ...
    // Auto-close após 120s
    setTimeout(() => {
      if (document.getElementById('nodus-drag-file-panel')) close();
    }, 120000);
  },
   ...
   * Cada plataforma tem um branch específico que tenta clicar no botão de anexo
   * para revelar o input[type="file"] oculto antes de tentar o upload.
   * Último fallback genérico cobre plataformas sem branch dedicado.
   */
  async uploadFileToplatform(file, platform) {
    try {
      // Helper: clicar num botão, aguardar e procurar o file input
      const clickAndWait = async (selector, delayMs = 500) => {
      ...
      // 1ª tentativa: file input já visível/acessível no DOM
      let input = document.querySelector('input[type="file"]');

      // ── ChatGPT ──────────────────────────────────────────────────────────────
      if (!input && platform === 'chatgpt') {
        input = await clickAndWait(
      ...
      // ── Gemini ────────────────────────────────────────────────────────────────
      // Menu em 2 passos: clicar '+' (canto inferior-esquerdo do INPUT) → menu
      // aparece (Enviar arquivos / Adicionar do Drive / Fotos / NotebookLM) →
      // clicar 'Enviar arquivos' → input[type=file] oculto é revelado.
      //
      // CRUCIAL: a resposta do assistente tem seu próprio menu '⋮' com
      // aria-label="Mostrar mais opções" (data-test-id="more-menu-button").
      // NÃO confundir com o '+' do input. Ancoramos a busca ao container do
      // input-area e procuramos o item dentro do painel do menu aberto.
      if (!input && platform === 'gemini') {
        // Helper: procura input[type=file] em todo DOM incl. shadow roots
        const deepFindFileInput = () => {
          const found = [];
          const walk = (root) => {
            if (!root) return;
            try {
              root.querySelectorAll && root.querySelectorAll('input[type="file"]').forEach(i => found.push(i));
        ...
        // Passo 1: file input já presente (mesmo escondido / shadow DOM)
        let inputs = deepFindFileInput();
        console.log('[Chains/Gemini] Initial file inputs (deep):', inputs.length);
        if (inputs.length > 0) {
          input = inputs.find(i => i.multiple) || inputs[0];
        }

        // Passo 2: localizar o container do input-area para ancorar a busca
        if (!input) {
          const inputAnchor =
          ...
          const inputContainer = inputAnchor
            ? (inputAnchor.closest('input-area-v2, .input-area, [class*="input-container"]') || inputAnchor.parentElement?.parentElement?.parentElement || inputAnchor.parentElement)
            : null;

          console.log('[Chains/Gemini] Input anchor:', inputAnchor?.tagName, '| container:', inputContainer?.tagName);

          // Passo 3: achar o botão '+' DENTRO do container do input
          // Excluir aria-labels que correspondem ao menu da RESPOSTA
          const isResponseMenuButton = (btn) => {
            const lbl = (btn.getAttribute('aria-label') || '').toLowerCase();
            const tid = (btn.getAttribute('data-test-id') || '').toLowerCase();
            return /mostrar\s+mais\s+op|more\s+options|show\s+more\s+opt|more-menu-button/.test(lbl + ' ' + tid);
          };

          const hasAddIcon = (btn) => {
            const mi = btn.querySelector('mat-icon');
            if (mi) {
              const fi = (mi.getAttribute('fonticon') || mi.getAttribute('data-mat-icon-name') || mi.textContent || '').trim().toLowerCase();
              if (/^add(\b|_)|^plus\b|^attach|^anexar/.test(fi)) return true;
            }
            return false;
          };
          ...
          // 3a) Dentro do container do input — prefer buttons com ícone 'add'
          if (inputContainer) {
            const nearbyBtns = Array.from(inputContainer.querySelectorAll('button'))
              .filter(b => b.offsetParent !== null && !isResponseMenuButton(b));

            menuTrigger = nearbyBtns.find(b => hasAddIcon(b)) || null;

            // Secundário: qualquer botão com aria-label/tooltip indicando adição
            if (!menuTrigger) {
              menuTrigger = nearbyBtns.find(b => {
                const lbl = (b.getAttribute('aria-label') || '').toLowerCase();
                const mtt = (b.getAttribute('mattooltip') || '').toLowerCase();
                return /adicionar|add\s+files|abrir\s+menu|anexar|upload|plus\b/.test(lbl + ' ' + mtt);
              });
            }

            if (menuTrigger) console.log('[Chains/Gemini] ✅ + button via input-container:', menuTrigger.getAttribute('aria-label') || menuTrigger.getAttribute('mattooltip') || menuTrigger.outerHTML.slice(0, 120));
          }

          // 3b) Global fallback — ainda excluindo o menu de resposta
          if (!menuTrigger) {
            const allBtns = Array.from(document.querySelectorAll('button'))
              .filter(b => b.offsetParent !== null && !isResponseMenuButton(b));
            menuTrigger = allBtns.find(b => hasAddIcon(b)) || null;
            if (menuTrigger) console.log('[Chains/Gemini] ✅ + button via global add-icon scan:', menuTrigger.outerHTML.slice(0, 120));
          }
          ...
          if (menuTrigger) {
            ...
            // Diagnóstico: dump de botões próximos ao input + alguns globais
            const nearBtns = inputContainer
              ? Array.from(inputContainer.querySelectorAll('button')).filter(b => b.offsetParent !== null)
              : [];
            const dump = nearBtns.slice(0, 20).map(b => ({
              ariaLabel: b.getAttribute('aria-label'),
              mattooltip: b.getAttribute('mattooltip'),
              testId: b.getAttribute('data-test-id'),
              iconName: b.querySelector('mat-icon')?.getAttribute('fonticon') || b.querySelector('mat-icon')?.textContent?.trim(),
              text: (b.textContent || '').trim().slice(0, 30)
            }));
            console.warn('[Chains/Gemini] ⚠️ + button não encontrado. Botões do input-area:', dump);
          }

          // Passo 4: achar 'Enviar arquivos' DENTRO do painel de menu aberto
          // (cdk-overlay-pane / mat-menu-panel / [role="menu"]) — NUNCA em texto
          // solto da página (como o próprio conteúdo da resposta).
          const openMenuPanels = Array.from(document.querySelectorAll(
            '.cdk-overlay-pane [role="menu"], .mat-mdc-menu-panel, .mat-menu-panel, [role="menu"]'
          )).filter(p => p.offsetParent !== null);

          const itemRegex = /^\s*(enviar\s+arquivo|adicionar\s+arquivo|carregar\s+arquivo|fazer\s+upload|upload\s+file|send\s+file|attach\s+file|escolher\s+arquivo|file\s+upload|carregar\s+do\s+computador|from\s+computer|browse)/i;

          let fileMenuItem = null;
          for (const panel of openMenuPanels) {
            const items = Array.from(panel.querySelectorAll(
              '[role="menuitem"], button, [role="option"], li, a'
            )).filter(el => el.offsetParent !== null);
            fileMenuItem = items.find(el => itemRegex.test((el.textContent || '').trim()));
            if (fileMenuItem) break;
          }

          if (fileMenuItem) {
            console.log('[Chains/Gemini] ✅ Menu item matched (dentro do painel):', (fileMenuItem.textContent || '').trim().slice(0, 60));
            fileMenuItem.click();
            await new Promise(r => setTimeout(r, 800));
          } else {
            const panelItems = [];
            openMenuPanels.forEach(panel => {
              Array.from(panel.querySelectorAll('[role="menuitem"], button')).forEach(it => {
                if (it.offsetParent !== null) panelItems.push((it.textContent || '').trim().slice(0, 50));
              });
            });
            console.warn('[Chains/Gemini] ⚠️ Item de upload não encontrado no menu aberto. Panels visíveis:', openMenuPanels.length, 'items:', panelItems);
          }

          // Passo 5: pegar o input[type=file] que acabou de aparecer
          inputs = deepFindFileInput();
          console.log('[Chains/Gemini] File inputs após click menu:', inputs.length);
          if (inputs.length > 0) {
            input = inputs.find(i => i.offsetParent !== null) || inputs.find(i => i.multiple) || inputs[0];
          }
        }
      }
      ...
      if (!input && platform === 'claude') {
        input = await clickAndWait(
          'button[aria-label*="Attach" i], button[aria-label*="Files" i], ' +
          'button[aria-label*="Upload" i], button[data-testid*="attach" i], ' +
          'button[aria-label*="file" i], label[data-testid*="attach" i]',
          500
        );
      }
      ...
      if (!input && platform === 'perplexity') {
        input = await clickAndWait(
          ...
          'button[aria-label*="file" i], label[class*="upload" i], ' +
      ...
      if (!input && platform === 'copilot') {
        input = await clickAndWait(
          ...
          'button[aria-label*="file" i], button[aria-label*="document" i]',
          500
        );
      }
      ...
      if (!input && platform === 'grok') {
        input = await clickAndWait(
          ...
          'label[for*="file" i], button[data-testid*="media" i]',
          500
        );
      }

      // ── DeepSeek ──────────────────────────────────────────────────────────────
      if (!input && platform === 'deepseek') {
        input = await clickAndWait(
          'button[aria-label*="upload" i], button[title*="upload" i], ' +
          'button[class*="upload" i], label[for*="file" i], ' +
          'button[aria-label*="attach" i], button[class*="attach" i]',
          500
        );
      }

      // ── Fallback genérico (qualquer plataforma ainda sem input) ───────────────
      // Tenta padrões comuns de botão de anexo antes de desistir
      if (!input) {
        input = await clickAndWait(
          'button[aria-label*="ttach" i], button[aria-label*="Upload" i], ' +
          'button[aria-label*="Media" i], button[aria-label*="File" i], ' +
          'button[data-testid*="attach" i], label[class*="upload" i], ' +
          'label[for*="file" i], button[class*="upload" i], ' +
          'button[title*="upload" i], button[title*="attach" i]',
          500
        );
      }

      // Último recurso: qualquer input[type="file"] no DOM, mesmo oculto
      if (!input) {
        input = document.querySelector('input[type="file"]');
      }

      if (!input) {
        console.warn('[Chains] File input não encontrado para plataforma:', platform);
        return false;
      }
  ...
  async deleteChain(chainId) {
    console.log('[Chains] 🗑️ Solicitando delete chain:', chainId);
  ...
  async executeDeleteChain(chainId) {
    ...
    try {
      ...
      if (response && response.ok) {
        console.log('[Chains] ✅ Telemetria: chain deletada registrada');
      } else {
  ...
  showRemoveNodeConfirmation(chainId, nodeId) {
    console.log('[Chains] Mostrando confirmação para remover node:', { chainId, nodeId });
  ...
  async executeRemoveNode(chainId, nodeId) {
    ...
    console.log('[RemoveNode] 🗑️ Iniciando remoção de node');
    console.log('[RemoveNode] chainId:', chainId);
    console.log('[RemoveNode] nodeId recebido:', nodeId);
    console.log('[RemoveNode] tipo do nodeId:', typeof nodeId);
    
    // Remover confirmação IMEDIATAMENTE
    const confirmEl = document.getElementById('nodus-remove-node-confirm');
    if (confirmEl) {
      console.log('[RemoveNode] Removendo popup de confirmação...');
      confirmEl.remove();
    }
    
    try {
      // Buscar chain atual
      console.log('[RemoveNode] Buscando chains...');
      const chains = await window.NodusChains.getAllChains();
      const chain = chains.find(c => c.id === chainId);
      ...
      console.log('[RemoveNode] ✅ Chain encontrada:', chain.name);
      console.log('[RemoveNode] Total de nodes na chain:', chain.nodes.length);
      
      // DEBUG: Mostrar estrutura dos nodes
      console.log('[RemoveNode] Estrutura dos nodes:');
      ...
      console.log('[RemoveNode] Procurando node com idea_id:', nodeId);
      ...
      if (nodeIndex === -1) {
        console.log('[RemoveNode] ⚠️ Não encontrou com idea_id, tentando ideaId...');
      ...
      console.log('[RemoveNode] ✅ Node encontrado no index:', nodeIndex);
      ...
      console.log('[RemoveNode] Chamando NodusChains.removeNodeFromChain...');
      console.log('[RemoveNode] Parâmetros: chainId =', chainId, ', ideaId =', nodeId);
      
      await window.NodusChains.removeNodeFromChain(chainId, nodeId);
      
      console.log('[RemoveNode] ✅ API removeNodeFromChain executada!');
      ...
      console.log('[RemoveNode] Nodes após remoção:', chainAfter.nodes.length);
      console.log('[RemoveNode] Diferença:', chain.nodes.length - chainAfter.nodes.length, 'node(s) removido(s)');
      ...
      // Re-render
      console.log('[RemoveNode] Chamando render()...');
      await this.render();
      ...
      console.log('[RemoveNode] ✅ Remoção concluída!');
      console.log('[RemoveNode] ═══════════════════════════════════════');
  ...
  async handleFullChatCapture() {
    try {
      console.log('[FullChat] Iniciando captura...');
      ...
      console.log('[FullChat] Platform detected:', platform);
      ...
      const handleCaptureEvent = async (event) => {
        ...
        console.log('[FullChat] ========== EVENTO RECEBIDO ==========');
        console.log('[FullChat] captureResult:', captureResult);
      ...
      console.log('[FullChat] ========== RESULTADO DA CAPTURA ==========');
      console.log('[FullChat] Type:', typeof captureResult);
      console.log('[FullChat] captureResult:', captureResult);
      console.log('[FullChat] ===========================================');
      
      // Se pending (modal aberto), aguardar evento
      if (captureResult.pending) {
        console.log('[FullChat] Modal aberto, aguardando escolha do usuário...');
  ...
  async processFullChatResult(captureResult, platform) {
    ...
    console.log('[FullChat] processFullChatResult chamado');
    console.log('[FullChat] captureResult.ok:', captureResult.ok);
    console.log('[FullChat] captureResult.error:', captureResult.error);
    console.log('[FullChat] captureResult.nodes:', captureResult.nodes);
    ...
    console.log('[FullChat] Captured nodes:', captureResult.nodes.length);
    ...
    console.log('[FullChat] Nome automático gerado:', autoName);
  ...
  async processFullChatCreation(chainName) {
    try {
      console.log('[FullChat] ========================================');
      console.log('[FullChat] Processando criação com nome:', chainName);
      console.log('[FullChat] pendingFullChatNodes:', this.pendingFullChatNodes);
      console.log('[FullChat] pendingFullChatPlatform:', this.pendingFullChatPlatform);
        ...
        for (const node of nodes) {
          ...
          for (const url of node.images) {
            ...
            try {
              ...
              console.log('[FullChat] 🖼️ Imagem salva no IndexedDB:', idbKey);
            } catch (err) {
      ...
      console.log('[FullChat] createResult recebido:', createResult);
      console.log('[FullChat] createResult.ok:', createResult.ok);
      console.log('[FullChat] createResult.chain:', createResult.chain);
      console.log('[FullChat] createResult.chain.id:', createResult.chain?.id);
      ...
      console.log('[FullChat] ✅ Chain criada:', chainId);
      console.log('[FullChat] ✅ Ideas salvas em fila virtual:', ideasCount);
      
      // ESTRATÉGIA DE RETRY: Buscar diretamente do storage até encontrar
      console.log('[FullChat] Aguardando chain aparecer no storage (máximo 3 tentativas)...');
      
      let chains = null;
      let newChainIndex = -1;
      
      for (let attempt = 1; attempt <= 3; attempt++) {
        console.log(`[FullChat] Tentativa ${attempt}/3 - aguardando ${attempt * 300}ms...`);
        ...
        console.log(`[FullChat] Tentativa ${attempt} - Total chains no storage:`, chains.length);
        console.log(`[FullChat] Tentativa ${attempt} - IDs:`, chains.map(c => c.id));
        
        // Procurar a chain
        newChainIndex = chains.findIndex(c => c.id === chainId);
        
        if (newChainIndex !== -1) {
          console.log(`[FullChat] ✅ Chain encontrada na tentativa ${attempt}! Index:`, newChainIndex);
          break;
        }
        
        console.log(`[FullChat] ⚠️ Tentativa ${attempt} - Chain ainda não encontrada`);
      }
      ...
      if (newChainIndex === -1) {
        ...
        console.log('[FullChat] ✅ Chain encontrada! Index:', newChainIndex);
        this.currentChainIndex = newChainIndex;
      }
      
      console.log('[FullChat] currentChainIndex definido como:', this.currentChainIndex);
      console.log('[FullChat] Chain selecionada:', chains[this.currentChainIndex]?.name, chains[this.currentChainIndex]?.id);
      
      // Limpar flags
      console.log('[FullChat] Limpando flags...');
      ...
      console.log('[FullChat] Recarregando ideas...');
      await this.loadAllIdeas();
      console.log('[FullChat] Total ideas após reload:', this.allIdeas.length);
      
      // Toast final
      this.showToast(`✅ ${ideasCount} mensagens capturadas! (Fila Virtual)`, 'success');
      
      console.log('[FullChat] Chamando render()...');
      ...
      console.log('[FullChat] ✅ Render concluído!');
      console.log('[FullChat] ========================================');
  ...
  async showPromoteModal(ideaId) {
    console.log('[Promote] 📤 Abrindo NodusPanelNQ para idea:', ideaId);
    ...
    console.log('[Promote] ✅ Idea encontrada na fila:', foundQueue);
    ...
    console.log('[Promote] 🚀 Abrindo NodusPanelNQ com dados preparados...');
    ...
    if (window.NodusPanelNQ && typeof window.NodusPanelNQ.open === 'function') {
      ...
      console.log('[Promote] ✅ NodusPanelNQ aberto!');
    } else {
  ...
  async executePromote(ideaId, sourceQueue, targetQueue) {
    console.log('[Promote] Executando promoção:', { ideaId, sourceQueue, targetQueue });
    ...
    try {
      ...
      console.log('[Promote] Idea encontrada:', idea.title);
      ...
      console.log('[Promote] ✅ Idea promovida com sucesso!');
  ...
  async openAttachmentsSidebar(chainId) {
    console.log('[Chains] 📎 Abrindo sidebar Attachments:', chainId);
  ...
  updateChainDragBarSelection() {
    ...
    console.log('[Chains] Selection updated:', selectedFiles.length, 'files');
  },
  ...
  async openNotesSidebar(chainId) {
    console.log('[Chains] 📝 Abrindo sidebar Notes:', chainId);
  ...
  async checkFullChatLimit() {
    console.log('[FullChat Gate] ✅ Ilimitado em todos os planos');
    return true;
  },
  ...
  async incrementFullChatCounter() {
    ...
    if (isPro) {
      // PRO não conta
      console.log('[FullChat Counter] 💎 PRO - Capture ilimitado');
    ...
    console.log(`[FullChat Counter] ✅ Incrementado: ${counter.count}/${counter.limit} (${remaining} restantes)`);
  ...
  async getFullChatCounter() {
    ...
    if (!data.nodus_fullchat_count) {
      ...
      console.log('[FullChat Counter] 🆕 Contador inicializado:', initial);
      return initial;
    }
  ...
  async checkMonthlyReset(counter) {
    ...
    if (now.getMonth() !== lastReset.getMonth() || 
      ...
      console.log('[FullChat Counter] 🔄 Reset mensal executado');
      console.log('[FullChat Counter] Anterior:', counter);
      ...
      console.log('[FullChat Counter] Novo:', resetCounter);
    }
  },
  ...
  async openFileTray() {
    ...
    console.log('[FileTray] Opened for chain:', chain.name);
  },
  ...
  async _trayHandleDragStart(e, chip) {
    ...
    try {
      ...
      console.log('[FileTray] Added', fileNames.length, 'files to dataTransfer');
    } catch (err) {
    ...
    console.log('[FileTray] Drag started:', fileNames.length, 'files');
  },
...
console.log('✅ NodusChainsUI carregado');
