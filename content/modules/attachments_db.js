...
const NodusAttachmentsDB = {
  ...
  async init() {
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        console.log('✅ [AttachmentsDB] DB inicializado');
        resolve(this.db);
      };
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains(this.STORE_NAME)) {
          ...
          console.log('🔧 [AttachmentsDB] Object store criado');
        }
      };
    });
  },
  ...
  async addFile(ideaId, file) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        request.onsuccess = () => {
          console.log('✅ [AttachmentsDB] Arquivo adicionado:', attachment.fileName);
  ...
  async getAttachmentsByIdeaId(ideaId) {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        console.log(`📎 [AttachmentsDB] ${attachments.length} arquivo(s) encontrado(s)`);
        resolve(attachments);
      };
  ...
  async getFile(attachmentId) {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        if (request.result) {
          console.log('📥 [AttachmentsDB] Arquivo recuperado:', request.result.fileName);
  ...
  async deleteFile(attachmentId) {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        console.log('🗑️ [AttachmentsDB] Arquivo removido:', attachmentId);
        resolve();
      };
  ...
  async deleteAllByIdeaId(ideaId) {
    ...
    console.log(`🗑️ [AttachmentsDB] ${attachments.length} arquivo(s) removido(s) da ideia ${ideaId}`);
  ...
  async downloadFile(attachmentId) {
    try {
      ...
      console.log('📥 [AttachmentsDB] Download iniciado:', attachment.fileName);
    } catch (error) {
  ...
  async getStorageInfo() {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        console.log(`📊 [AttachmentsDB] Total: ${totalFiles} arquivos, ${(totalSize / 1024 / 1024).toFixed(2)} MB`);
  ...
  async getFileBlob(attachmentId) {
    try {
      console.log('[AttachmentsDB] Getting blob for:', attachmentId);
      ...
      console.log(`📦 [AttachmentsDB] Blob created: ${attachment.fileName} (${blob.size} bytes)`);
  ...
  async getFileAsText(attachmentId) {
    try {
      console.log('[AttachmentsDB] Getting text for:', attachmentId);
      
      const blob = await this.getFileBlob(attachmentId);
      const text = await blob.text();
      
      console.log(`📝 [AttachmentsDB] Text retrieved: ${text.length} characters`);
...
(async () => {
  try {
    ...
    console.log('✅ [AttachmentsDB] Auto-initialized successfully');
  } catch (err) {
