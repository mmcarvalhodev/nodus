...
const NodusDashboardCards = {
    ...
    allIdeas: {},
    customTags: [],              // Tags persistidas independentemente de cards
    tagColors:  {},              // Mapa tag→cor persistido no storage
     ...
     */
    async init() {
        console.log('[Dashboard Cards] Initializing...');
    ...
    _autoSelectQueue() {
        ...
        if (first) {
            ...
            console.log('[Dashboard Cards] Auto-selected queue:', first);
        }
    },
    ...
    setupLicenseChangeListener() {
        ...
        console.log('[Dashboard Cards] 🔑 License change listener disabled (reload handles it)');
    },
    ...
    setupStateSyncListener() {
        ...
        console.log('[Dashboard Cards] 🔄 Configurando sincronização de estado...');
        
        // Sincronizar quando QUICK queue mudar
        window.NodusStateSync.subscribe('ideas_queue_quick', async (newValue) => {
            console.log('[Dashboard Cards] 🔄 QUICK queue atualizada em outra tab');
            if (this.currentQueue === 'quick') {
                await this.loadIdeas();
                console.log('[Dashboard Cards] ✅ Cards recarregados');
            }
        });
        
        // Sincronizar quando DEFAULT queue mudar
        window.NodusStateSync.subscribe('ideas_queue_default', async (newValue) => {
            console.log('[Dashboard Cards] 🔄 DEFAULT queue atualizada em outra tab');
        ...
        window.NodusStateSync.subscribe('ideas_queue_automatic', async (newValue) => {
            console.log('[Dashboard Cards] 🔄 AUTOMATIC queue atualizada em outra tab');
        ...
        console.log('[Dashboard Cards] ✅ Sincronização de estado ativa!');
    },
    ...
    setupAttachmentListener() {
        ...
        window.addEventListener('nodus-attachment-updated', async (e) => {
            ...
            console.log('[Dashboard] Attachment updated:', ideaId, action);
    ...
    async loadColumnLayout() {
        ...
        console.log('[Dashboard Cards] Column layout:', this.columnLayout);
    },
    ...
    async loadCardAnimation() {
        ...
        console.log('[Dashboard Cards] Card animation:', this.cardAnimation);
    },
    ...
    async saveColumnLayout(columns) {
        ...
        console.log('[Dashboard Cards] Saved column layout:', columns);
        ...
        if (modal) {
            ...
            console.log('[Dashboard Cards] Modal width set to', modal.style.width, 'cols=', columns);
        } else {
    ...
    async loadIdeas() {
        ...
        // Carregar tags e cores persistidas independentemente de cards
        this.customTags = data.nodus_custom_tags || [];
        this.tagColors  = data.nodus_tag_colors  || {};
    ...
    setupStorageListener() {
        chrome.storage.onChanged.addListener((changes, area) => {
            ...
            if (queueChanged) {
                console.log('[Dashboard Cards] Storage changed, refreshing...');
                ...
                if (currentTab === 'chains' && isPromoting) {
                    console.log('[Dashboard Cards] 🔒 Promoção detectada, mantendo view Chains');
        ...
        console.log('[Dashboard Cards] Storage listener setup complete');
    },
    ...
    async render(container) {
        console.log('[Dashboard Cards] Rendering...');
    ...
    createQueueTabs() {
        ...
        container.addEventListener('click', async (e) => {
            ...
            if (button && !button.disabled) {
                ...
                console.log('[Dashboard] Queue button clicked:', queueKey);
                console.log('[Dashboard] ANTES - currentQueue:', NodusDashboardCards.currentQueue);
                NodusDashboardCards.currentQueue = queueKey;
                console.log('[Dashboard] DEPOIS - currentQueue:', NodusDashboardCards.currentQueue);
                await NodusDashboardCards.loadIdeas();
                console.log('[Dashboard] Ideas recarregadas, chamando render...');
            ...
            if (prevBtn || nextBtn) {
                ...
                if (novoBotao) {
                    ...
                    console.log('[Dashboard] Arrow navigation to:', queueKey);
                    console.log('[Dashboard] ANTES - currentQueue:', NodusDashboardCards.currentQueue);
                    NodusDashboardCards.currentQueue = queueKey;
                    console.log('[Dashboard] DEPOIS - currentQueue:', NodusDashboardCards.currentQueue);
                    await NodusDashboardCards.loadIdeas();
    ...
    createIdeaCard(idea) {
        ...
        // Direct listener on note button — bypasses delegation, prevents double-fire
    ...
    getUniqueTags() {
        ...
        // Tags persistidas independentemente de cards
    ...
    attachEventListeners(container) {
        ...
        if (!this._cardActionsListenerAdded) {
            console.log('[Dashboard Cards] 🎯 Adicionando event listener delegado (ÚNICA VEZ)');
            ...
            container.addEventListener('click', (e) => {
                ...
                if (e.target.closest('.inline-attachment-item')) {
                    console.log('[Dashboard Cards] Click em inline-attachment-item, ignorando listener delegado');
                    return;
                }
                
                console.log('[Dashboard Cards] Click detected:', e.target);
                
                // Check if clicked on mode number
                const modeNumber = e.target.closest('.inject-mode-number');
                if (modeNumber) {
                    console.log('[Dashboard Cards] Mode number clicked, toggling inject mode');
                    this.toggleInjectMode();
                    return;
                }
                
                const actionBtn = e.target.closest('.card-action-btn, .card-action-btn-confirm, .card-action-btn-cancel, .card-attachment-btn, .card-expand-btn, .inject-btn-main, .inject-btn-mode');
                
                console.log('[Dashboard Cards] Action button found:', actionBtn);
                ...
                if (actionBtn) {
                    ...
                    console.log('[Dashboard Cards] Handling action:', { action, ideaId, queue });
                    
                    this.handleCardAction(action, ideaId, queue);
                } else {
                    console.log('[Dashboard Cards] No action button found for click');
                }
            });
            
            this._cardActionsListenerAdded = true;
            console.log('[Dashboard Cards] ✅ Event listener delegado registrado');
        } else {
            console.log('[Dashboard Cards] ⏭️ Event listener delegado JÁ existe, pulando');
        }
    },
    ...
    async handleCardAction(action, ideaId, queue) {
        console.log('[Dashboard Cards] Action:', action, ideaId, queue);
        ...
        if (!idea) {
            ...
            for (const [qName, ideas] of Object.entries(this.allIdeas)) {
                ...
                if (idea) {
                    console.log(`[Dashboard] Found idea in ${qName} queue`);
    ...
    async executeCardAction(action, idea, queueName) {
        console.log('[Dashboard Cards] 🎯 executeCardAction called:', { action, ideaId: idea?.id, queueName });
        ...
        switch(action) {
            case 'toggle-inject-mode':
                console.log('[Dashboard Cards] → toggle-inject-mode');
                this.toggleInjectModeMenu(idea.id);
                break;
            case 'toggle-expand':
                console.log('[Dashboard Cards] → toggle-expand');
                this.toggleCardExpand(idea.id);
                break;
            case 'inject-full':
                console.log('[Dashboard Cards] → inject-full');
                await this.injectIdea(idea, 'full');
                break;
            case 'inject-answer':
                console.log('[Dashboard Cards] → inject-answer');
                await this.injectIdea(idea, 'answer');
                break;
            case 'copy':
                console.log('[Dashboard Cards] → copy');
                await this.copyToClipboard(idea);
                break;
            case 'edit':
                console.log('[Dashboard Cards] → edit');
                this.editIdea(idea, queueName);
                break;
            case 'attachments':
                console.log('[Dashboard Cards] → attachments');
                this.openAttachments(idea.id, queueName);
                break;
            case 'notes':
                console.log('[Dashboard Cards] → notes');
                this.editNotes(idea, queueName);
                break;
            case 'delete':
                console.log('[Dashboard Cards] → delete');
                this.showDeleteConfirmation(idea.id);
                break;
            case 'delete-confirm':
                console.log('[Dashboard Cards] → delete-confirm');
                await this.deleteIdea(idea.id, queueName);
                this.hideDeleteConfirmation(idea.id);
                break;
            case 'delete-cancel':
                console.log('[Dashboard Cards] → delete-cancel');
                this.hideDeleteConfirmation(idea.id);
                break;
            default:
    ...
    async setInjectMode(mode, showToast = true) {
        ...
        console.log(`[Dashboard Cards] Inject mode changed to: ${mode} (${modeMap[mode]})`);
    ...
    async copyToClipboard(idea) {
        console.log('[Dashboard Cards] 📋 copyToClipboard called');
        ...
        console.log('[Dashboard Cards] Cross-Platform settings:', settings.crossPlatformInject);
        console.log('[Dashboard Cards] Idea platform:', idea.platform);
        console.log('[Dashboard Cards] Current platform:', this.detectPlatform());
        ...
        if (!settings.crossPlatformInject) {
            ...
            if (currentPlatform !== ideaPlatform) {
                console.log('[Dashboard Cards] ❌ BLOQUEANDO cópia Cross-Platform');
                if (window.NODUS_UI) {
                    window.NODUS_UI.showToast(
                        `🔒 Cross-Platform Inject desabilitado\nOrigem: ${ideaPlatform} → Destino: ${currentPlatform}\nHabilite nas Configurações ⚙️`,
                        'error'
    ...
    async editIdea(idea, queue) {
        console.log('[Dashboard Cards] Edit idea inline:', idea);
     ...
     */
    addToChain(idea) {
        console.log('[Dashboard Cards] Add to chain:', idea);
        if (window.NODUS_UI) {
    ...
    async openAttachments(ideaId, queueName) {
        console.log('[Dashboard Cards] Opening attachments inline for:', ideaId);
        ...
        // Load attachments
        const attachments = await window.NodusAttachmentsDB.getAttachmentsByIdeaId(ideaId);
        console.log('[Dashboard Cards] Loaded attachments:', attachments.length);
        ...
        if (!attachSection) {
            ...
            // Insert before actions (fallback: append to card)
            const actionsEl = card.querySelector('.card-actions');
    ...
    setupAttachmentsListeners(card, ideaId, queueName) {
        ...
        console.log('[Dashboard Cards] setupAttachmentsListeners CALLED');
        console.log('[Dashboard Cards] Card:', card);
        console.log('[Dashboard Cards] IdeaId:', ideaId);
        console.log('[Dashboard Cards] ========================================');
        ...
        console.log('[Dashboard Cards] Querying: .inline-attachment-item');
        console.log('[Dashboard Cards] Found', items.length, 'inline attachment items');
        
        if (items.length === 0) {
            console.error('[Dashboard Cards] ❌ NO ITEMS FOUND! Checking alternatives...');
            const allItems = card.querySelectorAll('.attachment-item');
            console.log('[Dashboard Cards] Found', allItems.length, '.attachment-item');
        }
        
        items.forEach((item, index) => {
            console.log('[Dashboard Cards] ----------------------------------------');
            console.log('[Dashboard Cards] Setting up item', index);
            console.log('[Dashboard Cards] Item element:', item);
            console.log('[Dashboard Cards] Item dataset:', item.dataset);
            console.log('[Dashboard Cards] Item classes:', item.className);
            
            // IMPORTANTE: Cursor pointer
            item.style.cursor = 'pointer';
            console.log('[Dashboard Cards] Cursor set to pointer');
            
            // Click to select
            const clickHandler = (e) => {
                console.log('[Dashboard Cards] ========================================');
                console.log('[Dashboard Cards] 🎯 CLICK HANDLER FIRED!');
                console.log('[Dashboard Cards] Item index:', index);
                console.log('[Dashboard Cards] Event target:', e.target);
                console.log('[Dashboard Cards] Current target:', e.currentTarget);
                console.log('[Dashboard Cards] Item element:', item);
                ...
                console.log('[Dashboard Cards] Is download button?', !!isDownloadBtn);
                console.log('[Dashboard Cards] Is delete button?', !!isDeleteBtn);
                
                if (isDownloadBtn || isDeleteBtn) {
                    console.log('[Dashboard Cards] ❌ Clicked on action button, IGNORING');
                    console.log('[Dashboard Cards] ========================================');
                    return;
                }
                ...
                console.log('[Dashboard Cards] Was selected?', wasSelected);
                console.log('[Dashboard Cards] Toggling...');
                console.log('[Dashboard Cards] Is selected NOW?', isSelected);
                ...
                if (isSelected) {
                    ...
                    console.log('[Dashboard Cards] ✅ Estilos de seleção aplicados!');
                } else {
                    // DESSELECIONADO
                    ...
                    console.log('[Dashboard Cards] ❌ Estilos de seleção removidos!');
                }
                
                console.log('[Dashboard Cards] Classes NOW:', item.className);
            ...
            console.log('[Dashboard Cards] ✅ Click handler attached to item', index);
            console.log('[Dashboard Cards] ----------------------------------------');
        });
        
        console.log('[Dashboard Cards] ========================================');
        console.log('[Dashboard Cards] setupAttachmentsListeners COMPLETE');
        console.log('[Dashboard Cards] ========================================');
        ...
        if (window.NodusAttachmentsDragBar) {
            console.log('[Dashboard Cards] Initializing drag bar...');
        ...
        if (addBtn && fileInput) {
            ...
            fileInput.addEventListener('change', async (e) => {
                ...
                console.log('[Dashboard Cards] Adding files:', files.length);
                
                for (const file of files) {
                    try {
                        await window.NodusAttachmentsDB.addFile(ideaId, file);
                        console.log('[Dashboard Cards] File added:', file.name);
                    } catch (error) {
    ...
    async editNotes(idea, queueName) {
        console.log('[Dashboard Cards] Opening notes inline for:', idea.id);
        ...
        // Ensure notes section is visible (in case card is near the bottom of the scroll area)
        setTimeout(() => {
    ...
    getTagColor(tagName) {
        ...
        // 1. Cores fixas por plataforma/categoria
        if (predefinedColors[upperTag]) return predefinedColors[upperTag];

        // 2. Cor persistida (atribuída no momento da criação)
        if (this.tagColors[upperTag]) return this.tagColors[upperTag];

        // 3. Fallback hash (tags antigas sem cor salva)
        let hash = 0;
        for (let i = 0; i < tagName.length; i++) {
            hash = tagName.charCodeAt(i) + ((hash << 5) - hash);
        }
        const fallback = ['#3b82f6','#8b5cf6','#ec4899','#10b981','#f59e0b','#ef4444','#06b6d4','#6366f1','#f97316','#14b8a6','#a855f7','#0ea5e9'];
        return fallback[Math.abs(hash) % fallback.length];
    },

    /**
     * Sorteia uma cor viva para uma nova tag e persiste no storage
     */
    async _assignTagColor(tagName) {
        const upperTag = tagName.toUpperCase();
        if (this.tagColors[upperTag]) return; // já tem cor

        const palette = [
            '#ef4444','#f97316','#f59e0b','#a3e635','#10b981','#14b8a6',
            '#06b6d4','#3b82f6','#6366f1','#8b5cf6','#a855f7','#ec4899',
            '#e11d48','#0284c7','#059669','#7c3aed','#db2777','#ea580c',
        ];
        // Escolhe aleatório mas evita repetir cor já usada se possível
        const usedColors = new Set(Object.values(this.tagColors));
        const available = palette.filter(c => !usedColors.has(c));
        const pool = available.length > 0 ? available : palette;
        const color = pool[Math.floor(Math.random() * pool.length)];

        this.tagColors[upperTag] = color;
        await chrome.storage.local.set({ nodus_tag_colors: this.tagColors });
    },
    ...
    async moveIdeaBetweenQueues(idea, fromQueue, toQueue) {
        console.log(`[Dashboard] Moving idea from ${fromQueue} to ${toQueue}`);
        ...
        console.log(`[Dashboard] Idea moved successfully!`);
    },
    ...
    setupInteractiveTags() {
        console.log('[Dashboard] Setting up interactive tags...');
        ...
        this._tagClickHandler = async (e) => {
            ...
            if (addBtn) {
                ...
                console.log('[Dashboard] Add button clicked, ideaId:', ideaId);
            ...
            if (tag) {
                ...
                console.log('[Dashboard] Tag clicked:', tagName, ideaId);
        ...
        console.log('[Dashboard] Interactive tags setup complete');
    },
        ...
        for (const [queueName, ideas] of Object.entries(this.allIdeas)) {
            ...
            if (idea) {
                ...
                if (quickIndex > 0) {
                    ...
                    console.log('[Dashboard] QUICK removed - no longer first tag');
                ...
                console.log('[Dashboard] Tags reordered:', newOrder);
    ...
    async checkAndMoveIdeaByTags(idea, currentQueue) {
        ...
        if (boundQueue !== `ideas_queue_${currentQueue}`) {
            ...
            console.log(`[Dashboard] Moving from ${currentQueue} to ${targetQueueName}`);
    ...
    openAddTagPopup(addBtn, ideaId) {
        console.log('[Dashboard] Opening add tag popup for idea:', ideaId);
        ...
        console.log('[Dashboard] Popup appended to body');
        ...
        console.log('[Dashboard] Card rect:', cardRect);
        console.log('[Dashboard] Button rect:', addBtnRect);
        
        // Position above the + button, centered on card
        const left = cardRect.left + (cardRect.width / 2);
        const top = addBtnRect.top - 10;
        
        console.log('[Dashboard] Popup position:', { left, top });
        ...
        setTimeout(() => {
            ...
            console.log('[Dashboard] Popup activated');
    ...
    async addTagToCard(ideaId, tagName) {
        ...
        for (const [queueName, ideas] of Object.entries(this.allIdeas)) {
            ...
            if (idea) {
                ...
                // Persistir tag no registro global + atribuir cor aleatória
                if (!this.customTags.includes(tagName)) {
                    this.customTags.push(tagName);
                    await chrome.storage.local.set({ nodus_custom_tags: this.customTags });
                }
                await this._assignTagColor(tagName);
    ...
    async clearCurrentQueue() {
        ...
        try {
            ...
            console.log(`[Dashboard Cards] Queue ${queueName} cleared!`);
    ...
    updateInlineDragBar(card) {
        ...
        console.log('[Dashboard Cards] Updating drag bar, selected:', selectedFiles.length);
    ...
    updateInlineActionButtons(card) {
        ...
        console.log('[Dashboard Cards] Updating inline buttons, selected:', count);
    ...
    async injectSelectedInline(card) {
        ...
        console.log('[Dashboard Cards] Injecting', selected.length, 'files inline');
    ...
    async copySelectedInline(card) {
        ...
        console.log('[Dashboard Cards] Copying', selected.length, 'files inline');
    ...
    async downloadSelectedInline(card) {
        ...
        console.log('[Dashboard Cards] Downloading', selected.length, 'files inline');
