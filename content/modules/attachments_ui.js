...
const NodusAttachmentsUI = {
    ...
    async open(ideaId, queueName) {
        console.log('[AttachmentsUI] ========================================');
        console.log('[AttachmentsUI] Opening modal for ideaId:', ideaId);
        console.log('[AttachmentsUI] Queue:', queueName);
        ...
        console.log('[AttachmentsUI] Idea found:', !!idea);
        ...
        // Load attachments
        console.log('[AttachmentsUI] Loading attachments...');
        const attachments = await window.NodusAttachmentsDB.getAttachmentsByIdeaId(ideaId);
        console.log('[AttachmentsUI] Attachments loaded:', attachments.length);

        // Check for file detection alert
        const showAlert = idea.hasGeneratedFile && attachments.length === 0;
        console.log('[AttachmentsUI] Show alert?', showAlert);

        // Render modal
        console.log('[AttachmentsUI] Rendering modal...');
        this.renderModal(idea, attachments, showAlert);
        console.log('[AttachmentsUI] Modal rendered!');
        console.log('[AttachmentsUI] ========================================');
    },
    ...
    renderModal(idea, attachments, showAlert) {
        console.log('[AttachmentsUI] renderModal called with:', { idea: idea.id, attachments: attachments.length, showAlert });
        
        // Remove existing modal
        const existing = document.getElementById('nodus-attachments-modal');
        if (existing) {
            console.log('[AttachmentsUI] Removing existing modal');
            existing.remove();
        }
        ...
        console.log('[AttachmentsUI] Modal HTML created, appending to body...');
        document.body.appendChild(modal);
        console.log('[AttachmentsUI] Modal appended, element:', modal);
        console.log('[AttachmentsUI] Modal in DOM?', document.getElementById('nodus-attachments-modal') !== null);
        ...
        requestAnimationFrame(() => {
            ...
            console.log('[AttachmentsUI] Added visible class');
        });
        
        console.log('[AttachmentsUI] renderModal complete');
    },
     ...
     */
    setupDragAndDrop() {
        console.log('[AttachmentsUI] Setting up drag and drop...');
        ...
        console.log('[AttachmentsUI] Found', items.length, 'draggable items');
        
        items.forEach((item, index) => {
            console.log('[AttachmentsUI] Setting up item', index);
            
            // Click to select (IMPORTANTE!)
            item.addEventListener('click', (e) => {
                console.log('[AttachmentsUI] Item clicked!', e.target);
                
                // Ignorar se clicou em botão de ação
                if (e.target.closest('.attachment-btn')) {
                    console.log('[AttachmentsUI] Clicked on action button, ignoring selection');
                    return;
                }
                
                // Toggle seleção
                item.classList.toggle('selected');
                console.log('[AttachmentsUI] Item selected?', item.classList.contains('selected'));
    ...
    updateDragBarSelection() {
        ...
        console.log('[AttachmentsUI] Selection updated:', selectedFiles.length, 'files');
    },
    ...
    handleDragStart(e, item) {
        ...
        console.log('🎯 [NODUS Drag] Iniciado:', draggedFiles.length, 'arquivo(s)');
        console.log('📋 [NODUS Drag] Arquivos:', draggedFiles.map(f => f.name));
    ...
    async addFile(file) {
        try {
            ...
            console.log('[AttachmentsUI] File added:', attachment.fileName);
    ...
    async injectSelected() {
        ...
        console.log('[AttachmentsUI] Injecting', selected.length, 'files');
    ...
    async copySelected() {
        ...
        console.log('[AttachmentsUI] Copying', selected.length, 'files');
    ...
    async downloadSelected() {
        ...
        console.log('[AttachmentsUI] Downloading', selected.length, 'files');
