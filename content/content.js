...
(async () => {
  ...
  console.log(`%c💡 NODUS v${NODUS_VERSION}`, 'color: #facc15; font-size: 16px; font-weight: bold;');
  ...
  if (typeof NodusI18n !== 'undefined') {
    ...
    console.log(`%c🌐 Idioma: ${NodusI18n.currentLang.toUpperCase()}`, 'color: #60a5fa;');
  }
  ...
  if (!platformName) {
    console.log('%c⚠️ Plataforma não suportada', 'color: #ef4444;');
    return;
  }
  
  console.log(`%c🎯 Plataforma: ${platformName.toUpperCase()}`, 'color: #10b981; font-weight: bold;');
  ...
  async function loadModule(path) {
    try {
      ...
      await import(url);
      console.log(`%c✅ ${path}`, 'color: #10b981; font-size: 11px;');
      return true;
    } catch (e) {
  ...
  console.log('%c📦 Carregando módulos...', 'color: #8b5cf6;');
  ...
  // Carregar engine runtime genérico (Fase 2: um runtime para todas as plataformas)
  // Specs são resolvidas em runtime via worker (nodus-worker) com fallback cascade:
  // storage → history → specs_bundled.json
  const engineLoaded = await loadModule('engines/runtime.js');

  if (!engineLoaded) {
    console.error(`❌ Falha ao carregar engine runtime (plataforma: ${platformName})`);
    return;
  }
  
  console.log('%c✅ NODUS inicializado!', 'color: #10b981; font-weight: bold; font-size: 14px;');
  ...
  // ═══════════════════════════════════════════════════════════════
  // KEEPALIVE — mantém o service worker vivo em dev mode pra que o
  // polling de spec (background.js → server local) nunca pare. SW MV3
  // dorme após ~30s de ociosidade; um sendMessage a cada 20s reseta o
  // timer. Em produção isso é desnecessário (alarm de 6h basta) mas
  // pra demo de hot-reload de spec é essencial.
  // ═══════════════════════════════════════════════════════════════
  setInterval(() => {
    try {
      chrome.runtime.sendMessage({ type: '_nodus_keepalive', t: Date.now() }, () => {
        void chrome.runtime.lastError; // ignora se SW recusar (já reativando)
      });
    } catch (_) { /* extension reload em curso */ }
  }, 20000);

  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    console.log('[Content] Mensagem recebida:', message.action || message.type);
    
    // 🔄 LICENSE UPDATED - Reload a página
    if (message.type === 'LICENSE_UPDATED') {
      console.log('[Content] 🔄 License atualizada, recarregando página...');
      location.reload();
      return;
    }

    // 🔄 SPEC UPDATED - background diz que tem versão nova; chama o
    // hook do runtime pra repuxar e atualizar window.__nodus_spec sem reload.
    // Isso fecha o ciclo Fase 2: edita spec no server → background pega →
    // notifica todas as tabs → runtime aplica → próxima sanity check vê o fix.
    if (message.type === 'spec_updated') {
      console.log(`[Content] 🔄 Spec atualizada para v${message.version} — recarregando no runtime...`);
      try {
        if (typeof window.__nodus_reload_spec === 'function') {
          window.__nodus_reload_spec().then(
            (info) => console.log('[Content] ✅ Spec aplicada:', info),
            (err)  => console.warn('[Content] ❌ Falha ao aplicar spec:', err)
          );
        } else {
          console.warn('[Content] window.__nodus_reload_spec não existe — runtime não inicializado?');
        }
      } catch (e) {
        console.warn('[Content] Erro ao aplicar spec_updated:', e);
      }
      return;
    }
    ...
    if (message.action === 'open_dashboard') {
      try {
        console.log('[Content] Abrindo Dashboard...');
    ...
    if (message.action === 'open_settings') {
      try {
        console.log('[Content] Abrindo Settings...');
    ...
    if (message.action === 'openPanelNQ') {
      try {
        console.log('[Content] Abrindo Panel NQ com dados:', message.ideaData);
    ...
    if (message.action === 'injectText') {
      try {
        ...
        console.log(`[Content] Inject mode: ${injectMode}`);

        // ── FASE 2: spec é a fonte de verdade ──────────────────────
        // Tenta primeiro ler `spec.input.selectors` exposto pelo runtime
        // em window.__nodus_spec. Se a spec tiver os selectors definidos,
        // usa eles — assim quando o worker servir uma spec atualizada
        // (ChatGPT/Claude/etc mudou o DOM), o paste passa a funcionar
        // SEM precisar republicar a extensão.
        //
        // Hardcoded fallback abaixo é safety net pra:
        //   1. Spec ainda não carregada (cold start, race condition)
        //   2. Plataforma nova adicionada via spec sem ter sido prevista aqui
        //   3. spec.input.selectors estar vazio/malformado
        ...
        // Resolve selectors: spec primeiro, fallback depois.
        // Concatena os 2 (spec na frente) pra maximizar chance de match
        // mesmo se a spec ainda não conhecer um seletor que o hardcoded tem.
        const specSelectors = window.__nodus_spec?.input?.selectors;
        const fallbackSelectors = HARDCODED_FALLBACK[platformName] || [];
        const selectors = (Array.isArray(specSelectors) && specSelectors.length > 0)
          ? [...specSelectors, ...fallbackSelectors.filter(s => !specSelectors.includes(s))]
          : fallbackSelectors;

        const sourceLabel = (Array.isArray(specSelectors) && specSelectors.length > 0)
          ? `spec v${window.__nodus_spec_version || '?'} (+ fallback)`
          : 'hardcoded fallback (spec ausente)';
        console.log(`[Content] Inject — selectors source: ${sourceLabel}`);

        if (!selectors || selectors.length === 0) {
          console.error('[Content] Nenhum selector disponível para plataforma:', platformName);
        ...
        for (const selector of selectors) {
          ...
          if (input) {
            console.log(`[Content] Input encontrado com: ${selector}`);
            break;
          }
        }
        ...
        if (!input) {
          ...
          console.log('[Content] Tentou seletores:', selectors);
        ...
        if (input.tagName === 'RICH-TEXTAREA') {
          ...
          if (innerInput) {
            input = innerInput;
            console.log('[Content] Input contentEditable encontrado dentro de rich-textarea');
          }
        }
        ...
        if (input.tagName === 'TEXTAREA') {
          ...
          if (platformName === 'perplexity' && input.hasAttribute('data-lexical-editor')) {
            console.log('[Content] Detectado Perplexity Lexical - usando execCommand');
            ...
            console.log(`[Content] ✅ Texto injetado no Perplexity Lexical (${injectMode})`);
          ...
          if (injectMode === 'formatted') {
            ...
            if (text.includes('#') || text.includes('**') || text.includes('```')) {
              ...
              console.log('[Content] 🎨 Markdown convertido para HTML');
              console.log('[Content] Original length:', text.length);
              console.log('[Content] HTML length:', htmlContent.length);
              console.log('[Content] HTML preview:', htmlContent.substring(0, 200));
            }
            ...
            console.log(`[Content] ✅ Texto injetado (Markdown raw)`);
            sendResponse({ ok: true, message: 'Texto injetado' });
            return true;
            ...
            console.log(`[Content] ✅ Texto injetado (Plain text)`);
            sendResponse({ ok: true, message: 'Texto injetado' });
            return true;
          }
        ...
        console.log(`[Content] ✅ Texto injetado (${injectMode})`);
        sendResponse({ ok: true, message: 'Texto injetado' });
  ...
  console.log('[Content] Listener de inject ativo');
})();
