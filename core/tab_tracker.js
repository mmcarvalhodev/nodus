...
const NodusTabTracker = {
  ...
  async registerTab() {
    try {
      ...
      if (!chrome.storage) {
        console.log('[TabTracker] Contexto invalidado, não é possível registrar');
        return null;
      }
      ...
      console.log('[TabTracker] Aba registrada:', this.currentTabId, this.currentPlatform);
      ...
      if (error.message && error.message.includes('Extension context invalidated')) {
        console.log('[TabTracker] Contexto invalidado durante registro');
        return null;
      }
  ...
  async updateHeartbeat() {
    try {
      ...
      if (!chrome.storage) {
        ...
        console.log('[TabTracker] Contexto invalidado, parando heartbeat');
        this.stopHeartbeat();
        return;
      }
  ...
  async unregisterTab() {
    try {
      ...
      // Verificar se o contexto ainda é válido
      if (!chrome.storage) {
        console.log('[TabTracker] Contexto invalidado, não é possível remover aba');
        return;
      }
      ...
      await chrome.storage.local.set({ [this.STORAGE_KEY]: activeTabs });
      console.log('[TabTracker] Aba removida:', this.currentTabId);
    } catch (error) {
      // Silenciar erro de contexto invalidado
      if (error.message && error.message.includes('Extension context invalidated')) {
        console.log('[TabTracker] Contexto invalidado ao remover aba');
        return;
      }
  ...
  async cleanupOldTabs() {
    try {
      ...
      if (cleaned) {
        await chrome.storage.local.set({ [this.STORAGE_KEY]: activeTabs });
        console.log('[TabTracker] Abas antigas limpas');
      }
    } catch (error) {
      // Silenciar erro de contexto invalidado
      if (error.message && error.message.includes('Extension context invalidated')) {
  ...
  stopHeartbeat() {
    if (this.heartbeatInterval) {
      ...
      console.log('[TabTracker] Heartbeat parado');
    }
  },
  ...
  async init() {
    ...
    console.log('[TabTracker] Inicializado');
  }
};
