...
window.NodusStateSync = {
  ...
  init() {
    if (this.isInitialized) {
      console.log('[StateSync] ⚠️ Já inicializado');
      return;
    }
    
    console.log('[StateSync] 🚀 Inicializando sincronização de estado...');
    
    // Listener global para mudanças no storage
    chrome.storage.onChanged.addListener((changes, areaName) => {
      if (areaName !== 'local') return;
      
      console.log('[StateSync] 🔄 Mudanças detectadas:', Object.keys(changes));
      
      // Processar cada mudança
      for (const [key, { oldValue, newValue }] of Object.entries(changes)) {
        if (this.WATCHED_KEYS.includes(key)) {
          console.log(`[StateSync] 📦 ${key} alterado`);
    ...
    console.log('[StateSync] ✅ Sistema de sincronização ativo!');
  },
  ...
  subscribe(key, callback) {
    ...
    console.log(`[StateSync] 📡 Inscrito em: ${key}`);
    ...
    return () => {
      ...
      if (index > -1) {
        ...
        console.log(`[StateSync] 🔕 Desinscrito de: ${key}`);
      }
    };
  },
  ...
  notifyChange(key, newValue, oldValue) {
    ...
    if (relevantListeners.length === 0) {
      console.log(`[StateSync] ℹ️ Nenhum listener para: ${key}`);
      return;
    }
    
    console.log(`[StateSync] 📢 Notificando ${relevantListeners.length} listener(s) de: ${key}`);
  ...
  syncCardsView() {
    return this.subscribe('ideas_queue_quick', async (newValue) => {
      console.log('[StateSync] 🔄 Atualizando Cards View...');
      ...
      if (window.NodusDashboardCards && window.NodusDashboardCards.isOpen) {
        ...
        console.log('[StateSync] ✅ Cards View atualizado');
      }
    });
  },
  ...
  syncChainsView() {
    return this.subscribe('nodus_chains', async (newValue) => {
      console.log('[StateSync] 🔄 Atualizando Chains View...');
      ...
      if (window.NodusDashboardChains && window.NodusDashboardChains.isOpen) {
        ...
        console.log('[StateSync] ✅ Chains View atualizado');
      }
    });
  },
  ...
  syncQueues() {
    ...
    ['ideas_queue_quick', 'ideas_queue_default', 'ideas_queue_automatic'].forEach(queueKey => {
      const unsubscribe = this.subscribe(queueKey, async (newValue) => {
        console.log(`[StateSync] 🔄 Queue alterada: ${queueKey}`);
  ...
  async broadcastToTabs(action, data = {}) {
    try {
      ...
      console.log(`[StateSync] 📡 Broadcasting "${action}" para ${tabs.length} tabs`);
  ...
  onBroadcast(callback) {
    const listener = (message) => {
      if (message.action === 'state_sync_broadcast') {
        console.log(`[StateSync] 📨 Broadcast recebido: ${message.syncAction}`);
   ...
   */
  async forceSync(key) {
    console.log(`[StateSync] 🔄 Forçando sync de: ${key}`);
...
console.log('[StateSync] 📦 Módulo carregado');
