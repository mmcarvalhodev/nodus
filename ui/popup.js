...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (typeof NodusI18n !== 'undefined') {
    ...
    console.log('[Popup] i18n:', NodusI18n.currentLang);
  }
...
async function checkTelemetry() {
  try {
    ...
    if (mode === undefined) {
      console.log('[Popup] Telemetria não configurada, usando default (ON)');
      return true;
    }
...
async function loadStats() {
  try {
    ...
    console.log('[Popup] Filas virtuais encontradas:', virtualQueueKeys);
...
function formatTime(date) {
  console.log('[Popup] formatTime chamado com:', date, typeof date);
  
  // Se não é um objeto Date, tentar converter
  if (!(date instanceof Date)) {
    date = new Date(date);
    console.log('[Popup] Convertido para Date:', date);
  }
  ...
  console.log('[Popup] Horário formatado:', formatted);
  return formatted;
}
...
async function toggleTelemetry() {
  try {
    ...
    // ORDEM CRÍTICA: emite o evento de toggle ANTES de persistir a flag.
    // Se invertêssemos (marca off → tenta enviar), o sendBatch ia respeitar a
    // flag nova e descartar o evento — opt-out viraria fantasma.
    // O handler no background faz flush IMEDIATO desse evento.
    try {
      await chrome.runtime.sendMessage({
        type: 'telemetry_toggle',
        payload: { enabled, source: 'popup' }
      });
    } catch (e) {
      console.warn('[Popup] toggle event send falhou:', e?.message);
    }
    ...
    console.log(`[Popup] Telemetria ${enabled ? 'ativada' : 'desativada'}`);
